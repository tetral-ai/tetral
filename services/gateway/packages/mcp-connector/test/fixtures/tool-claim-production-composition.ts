...
if (bridgeAddress === undefined || gatewayTokenPath === undefined || runtimeTokenPath === undefined ||
  toolUseEventIdArgument === undefined || cleanupToolUseEventIdArgument === undefined) {
  throw new Error("usage: tool-claim-production-composition <bridge-address> <gateway-token-path> <runtime-token-path> <tool-use-event-id> <cleanup-tool-use-event-id>");
}
...
class TakeoverMcpClient implements McpClient {
  ...
  async callTool(input: Parameters<McpClient["callTool"]>[0]) {
    if (input.input["mode"] === "cleanup") {
      ...
      return { content: [{ type: "text" as const, text: "reacquired claimant" }] };
    }
    const callNumber = this.calls.length + 1;
    this.calls.push(`${callNumber}:${input.toolName}:${JSON.stringify(input.input)}`);
    if (callNumber === 1) {
...
try {
  await serviceForClaim("claim_mcp_production_cleanup_failed").runMcpTool(runRequest(cleanupToolUseEventId), metadata);
} catch (error) {
  cleanupFailureCode = typeof error === "object" && error !== null && "code" in error
    ? Number((error as { readonly code: unknown }).code)
    : -1;
}
const cleanupReacquired = await serviceForClaim("claim_mcp_production_cleanup_reacquired")
...
if (runtimeResult.type !== "completed") throw new Error(`MCP Runtime mapping returned ${JSON.stringify(runtimeResult)}`);
const writer = new BridgeAPIEventWriter({ address: bridgeAddress, tokenPath: runtimeTokenPath, sleep: async () => undefined });
...
function serviceForClaim(claimId: string): McpConnectorServiceShell {
  const authenticator: McpAuthenticator = {
    authenticate: async ({ metadata: requestMetadata }) =>
      requestMetadata.get("authorization").some((value) => String(value).toLowerCase() === "bearer mcp-production-runtime-token")
        ? { ok: true, serviceAccount: { namespace: "tetral", name: "runtime", podUid: runtimePodUid } }
        : { ok: false, code: "Unauthenticated", message: "composition caller token rejected" },
  };
...
function signedBindingToken(): string {
  ...
  const signature = createHmac("sha256", bindingKey).update(payloadPart).digest("base64url");
