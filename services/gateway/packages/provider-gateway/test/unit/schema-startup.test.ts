...
describe("provider-gateway schema verification", () => {
  test("accepts the exact current registry through SELECT-only SQL", async () => {
    ...
    const sql = schemaSQL(queries, [
      [{ exists: true }],
      [
        { version: 1, checksum: "afb5f913c2f846d7c559965ec20869c5aec84a984f708b0218ddbadc0190ab2a" },
      ],
    ]);
  ...
  for (const testCase of [
    ...
    { name: "duplicate", responses: [[{ exists: true }], [{ version: 1, checksum: "afb5f913c2f846d7c559965ec20869c5aec84a984f708b0218ddbadc0190ab2a" }, { version: 1, checksum: "afb5f913c2f846d7c559965ec20869c5aec84a984f708b0218ddbadc0190ab2a" }]], kind: "schema_history_duplicate" },
    ...
    test(`fails closed for ${testCase.name} history`, async () => {
      const sql = schemaSQL([], testCase.responses.map((response) => [...response]));
      try {
  ...
  test("turns malformed driver results into a typed public-safe error", async () => {
    ...
    try {
      ...
      expect(error).toBeInstanceOf(SchemaVerificationError);
      expect((error as SchemaVerificationError).kind).toBe("schema_history_malformed");
      expect(String(error)).not.toContain("SELECT");
    }
  });
...
function schemaSQL(queries: string[], responses: unknown[][]): SchemaSQL & { close: () => Promise<void> } {
  ...
  query.close = async () => { queries.push("close"); };
  return query;
}
