...
import { OpenAICodexResponsesEndpoint, OpenAIOAuthDummyAPIKey } from "../../src/providers/openai-oauth.js";
...
const OpenAIFixtureUrl = new URL("fixtures/openai-gpt-5.5-responses-live-2026-07-06.sse", import.meta.url);
const OpenAIGPT56SolFixtureUrl = new URL("fixtures/openai-gpt-5.6-sol-responses-live-2026-07-19.sse", import.meta.url);
const OpenAIGPT56SolOAuthFixtureUrl = new URL("fixtures/openai-gpt-5.6-sol-oauth-responses-live-2026-07-19.sse", import.meta.url);
const OpenAIDisconnectFixtureUrl = new URL("fixtures/openai-gpt-5.5-responses-disconnect-prefix-2026-07-06.sse", import.meta.url);
const OpenAIHttpErrorFixtureUrl = new URL("fixtures/openai-gpt-5.5-responses-http-error-2026-07-06.txt", import.meta.url);
const AnthropicFixtureUrl = new URL("fixtures/anthropic-claude-opus-4-8-live-2026-07-03.sse", import.meta.url);
const AnthropicFableFixtureUrl = new URL("fixtures/anthropic-claude-fable-5-live-2026-07-19.sse", import.meta.url);
const AnthropicCacheHitFixtureUrl = new URL("fixtures/anthropic-claude-opus-4-8-cache-hit-2026-07-09.sse", import.meta.url);
const AnthropicDisconnectFixtureUrl = new URL("fixtures/anthropic-claude-opus-4-8-disconnect-prefix-2026-07-03.sse", import.meta.url);
const KimiK3FixtureUrl = new URL("fixtures/moonshotai-kimi-k3-live-2026-07-19.sse", import.meta.url);
const KimiK3ToolFixtureUrl = new URL("fixtures/moonshotai-kimi-k3-tool-live-2026-07-19.sse", import.meta.url);
const KimiCacheHitFixtureUrl = new URL("fixtures/moonshotai-kimi-k3-cache-hit-2026-07-19.sse", import.meta.url);
const KimiDisconnectFixtureUrl = new URL("fixtures/moonshotai-kimi-k3-disconnect-prefix-2026-07-19.sse", import.meta.url);
const KimiHttpErrorFixtureUrl = new URL("fixtures/moonshotai-kimi-k3-http-error-2026-07-19.txt", import.meta.url);
const DeepSeekFixtureUrl = new URL("fixtures/deepseek-deepseek-v4-pro-live-2026-07-06.sse", import.meta.url);
const DeepSeekDisconnectFixtureUrl = new URL("fixtures/deepseek-deepseek-v4-pro-disconnect-prefix-2026-07-06.sse", import.meta.url);
const DeepSeekHttpErrorFixtureUrl = new URL("fixtures/deepseek-deepseek-v4-pro-http-error-2026-07-06.txt", import.meta.url);
const ZaiFixtureUrl = new URL("fixtures/zai-glm-5.2-live-2026-07-04.sse", import.meta.url);
const ZaiDisconnectFixtureUrl = new URL("fixtures/zai-glm-5.2-disconnect-prefix-2026-07-04.sse", import.meta.url);
const ZaiHttpErrorFixtureUrl = new URL("fixtures/zai-glm-5.2-http-error-2026-07-04.txt", import.meta.url);
const AnthropicBetaHeader = "structured-outputs-2025-11-13,interleaved-thinking-2025-05-14,fine-grained-tool-streaming-2025-05-14";
const ApprovalReviewerOutputSchemaUrl = new URL("../../../../../agent-runtime/packages/runtime-pod/src/assets/approval-reviewer-output-schema.json", import.meta.url);
...
describe("provider golden fixture provenance", () => {
  test("requires date and model provenance on every JSON golden artifact", async () => {
    const files = await collectGoldenJsonFixturePaths(new URL(".", import.meta.url));
  ...
  test("pins every approved model's output-cap boundary on the provider wire", async () => {
    const cases = [
      {
        request: { ...openAIGoldenRequest(), limits: { maxOutputTokens: 128_001, timeoutMs: 60_000 } },
      ...
      {
        request: { ...openAIGPT56SolGoldenRequest(), limits: { maxOutputTokens: 128_001, timeoutMs: 60_000 } },
        credential: sessionOpenAICredential(),
      ...
      {
        request: { ...anthropicGoldenRequest(), limits: { maxOutputTokens: 128_001, timeoutMs: 60_000 } },
      ...
      {
        request: { ...anthropicFableGoldenRequest(), limits: { maxOutputTokens: 128_001, timeoutMs: 60_000 } },
        credential: sessionAnthropicCredential(),
      ...
      {
        request: { ...deepSeekGoldenRequest(), limits: { maxOutputTokens: 384_001, timeoutMs: 60_000 } },
      ...
      {
        request: { ...kimiK3GoldenRequest(), limits: { maxOutputTokens: 131_073, timeoutMs: 60_000 } },
      ...
      {
        request: { ...zaiGoldenRequest(), limits: { maxOutputTokens: 131_073, timeoutMs: 60_000 } },
    ...
    for (const scenario of cases) {
      const mock = createMockAnthropicServer(await readFile(scenario.fixture, "utf8"));
      const registry = new ProviderClientRegistry({ fetch: mock.fetch });
      try {
        await collectEvents(registry.stream({ request: scenario.request, credential: scenario.credential }));
        expect(mock.requests).toHaveLength(1);
        if (scenario.expected === undefined) {
          expect(mock.requests[0]!.body).not.toHaveProperty(scenario.field);
        } else {
          expect(mock.requests[0]!.body[scenario.field]).toBe(scenario.expected);
        }
      } finally {
        await mock.close();
      }
  ...
  test("emits plain-text attachments as Anthropic document text blocks on both Anthropic-family routes", async () => {
    ...
    for (const scenario of scenarios) {
      ...
      try {
        ...
        expect(JSON.stringify(messages)).not.toContain("ERROR: cannot read attachment");
      } finally {
        await mock.close();
      }
    }
...
describe("expanded provider catalog golden wire paths", () => {
  test("replays Claude Fable 5 through the frozen Anthropic rule family", async () => {
    ...
    expect(fixture).toContain("recorded 2026-07-19 model_id=anthropic/claude-fable-5 source=real-provider");
    ...
    try {
      ...
      expect(mock.requests).toHaveLength(1);
      await expectCapturedRequestFixture(mock.requests[0]!, "anthropic-fable/basic-turn.http.json");
      await expectProviderEventsFixture(events, "anthropic-fable/basic-turn.events.json");
    } finally {
      await mock.close();
    }
  });
  ...
  test("replays Kimi K3 native thinking without injecting a thinking budget", async () => {
    ...
    expect(fixture).toContain("recorded 2026-07-19 model_id=moonshotai/kimi-k3 source=real-provider");
    expectFixtureHasNoProviderSecrets(fixture);
    const mock = createMockAnthropicServer(fixture);
    const registry = new ProviderClientRegistry({ fetch: mock.fetch });
    try {
      ...
      await expectCapturedRequestFixture(mock.requests[0]!, "moonshotai-k3/basic-turn.http.json");
      await expectProviderEventsFixture(events, "moonshotai-k3/basic-turn.events.json");
    } finally {
      await mock.close();
    }
  });
  ...
  test("replays Kimi K3 thinking and tool_use history in transport-native form", async () => {
    ...
    expect(fixture).toContain("model_id=moonshotai/kimi-k3 source=real-provider");
    expect(fixture).toContain("case=tool-use");
    expectFixtureHasNoProviderSecrets(fixture);
    const mock = createMockAnthropicServer(fixture);
    const registry = new ProviderClientRegistry({ fetch: mock.fetch });
    try {
      const firstTurn = await collectEvents(registry.stream({
        request: kimiK3ToolGoldenRequest(),
        credential: sessionKimiCredential(),
      }));
      const reasoningText = firstTurn.map((event) => event.reasoning?.text ?? "").join("");
      const signature = firstTurn
        .map((event) => event.reasoning?.metadataJson ?? "{}")
        .map((metadataJson) => JSON.parse(metadataJson) as { anthropic?: { signature?: string } })
        .find((metadata) => metadata.anthropic?.signature !== undefined)
        ?.anthropic?.signature;
      const toolCall = firstTurn.find((event) => event.toolCall !== undefined)?.toolCall;
...
describe("OpenAI Responses golden wire path", () => {
  test("captures GPT-5.6 Sol through the official API-key supply", async () => {
    ...
    try {
      ...
      await expectCapturedRequestFixture(captured, "openai-gpt56-sol-official/basic-turn.http.json");
      await expectProviderEventsFixture(events, "openai-gpt56-sol-official/basic-turn.events.json");
    } finally {
      await mock.close();
    }
  });
  ...
  test("captures GPT-5.6 Sol through the ChatGPT OAuth supply", async () => {
    ...
    try {
      const events = await collectEvents(registry.stream({
        request: openAIGPT56SolGoldenRequest("sesn_live_gpt56_sol_oauth_fixture"),
        credential: sessionOpenAIOAuthCredential(),
      }));

      expect(mock.requests).toHaveLength(1);
      const captured = mock.requests[0]!;
      expect(captured.pathname).toBe(new URL(OpenAICodexResponsesEndpoint).pathname);
      ...
      expect(captured.body).not.toHaveProperty("max_output_tokens");
      await expectCapturedRequestFixture(captured, "openai-gpt56-sol-oauth/basic-turn.http.json");
      await expectProviderEventsFixture(events, "openai-gpt56-sol-oauth/basic-turn.events.json");
    } finally {
      await mock.close();
    }
  });
  ...
  test("captures the official Responses request shape and raises the replayed fixture", async () => {
    ...
    expect(fixture).toContain("recorded 2026-07-08 model_id=openai/gpt-5.5 source=real-provider");
    expectFixtureHasNoProviderSecrets(fixture);
    const mock = createMockAnthropicServer(fixture);
    const registry = new ProviderClientRegistry({ fetch: mock.fetch });
    try {
      ...
      expect(mock.requests).toHaveLength(1);
      const captured = mock.requests[0]!;
      await expectCapturedRequestFixture(captured, "openai/basic-turn.http.json");
      ...
      expect(captured.body.input).toEqual([
        {
          role: "developer",
          content: "Keep visible output minimal and call tools exactly as requested.",
        },
        {
          role: "user",
          content: [{
      ...
      await expectProviderEventsFixture(events, "openai/basic-turn.events.json");
  ...
  test("captures the OAuth Responses rewrite, header swap, and instructions", async () => {
    ...
    try {
      ...
      expect(captured.headers.authorization).not.toBe(`Bearer ${OpenAIOAuthDummyAPIKey}`);
      expect(captured.body).toMatchObject({
        model: "gpt-5.5",
        stream: true,
        store: false,
        instructions: "Keep visible output minimal and call tools exactly as requested.",
        prompt_cache_key: "sesn_live_openai_fixture",
      });
      expect(JSON.stringify(captured.body.input)).not.toContain("\"id\"");
      expect(captured.body.input).toEqual([
  ...
  test("turns a mid-stream OpenAI Responses transport disconnect into a retryable stream failure after partial events", async () => {
    ...
    try {
      ...
      expect(result.events.map((event) => event.reasoning?.text ?? "").join("")).toContain("Evaluating tool response");
  ...
  test("classifies a recorded OpenAI Responses HTTP provider error without platform-key fallback", async () => {
    ...
    try {
      ...
      expect((result.error as ProviderKeyFailureError).classification).toMatchObject({
        action: "fail-fast",
        providerError: {
...
describe("Anthropic golden wire path", () => {
  test("lowers approval reviewer schema onto Anthropic provider-native output format", async () => {
    const outputSchemaJson = await readFile(ApprovalReviewerOutputSchemaUrl, "utf8");
    ...
    try {
      ...
      expect(JSON.stringify(captured.body.tools)).not.toContain('"name":"json"');
      expect(events.map((event) => event.text?.text ?? "").join("")).toContain('"outcome":"allow"');
    } finally {
      await mock.close();
    }
  });
  ...
  test("captures the AI SDK HTTP request and raises the recorded Anthropic SSE fixture", async () => {
    ...
    expect(fixture).toContain("recorded 2026-07-03 model_id=anthropic/claude-opus-4-8 source=real-provider");
    expectFixtureHasNoProviderSecrets(fixture);
    const mock = createMockAnthropicServer(fixture);
    const registry = new ProviderClientRegistry({ fetch: mock.fetch });
    try {
      ...
      const reasoningText = events.map((event) => event.reasoning?.text ?? "").join("");
      expect(reasoningText).toContain("271828");
      expect(reasoningText).toContain("585987");
      expect(events.some((event) => event.reasoning?.metadataJson.includes("signature"))).toBe(true);
      expect(events.map((event) => event.text?.text ?? "").join("")).toBe("reading now.");
      expect(events.map((event) => event.toolInput?.text ?? "").join("")).toBe("{\"path\": \"/workspace/app.ts\"}");
      ...
      expect(events[13]?.finish?.usage?.providerUsageJson).toContain("noCacheTokens");
      await expectProviderEventsFixture(events, "anthropic/basic-turn.events.json");
    } finally {
      await mock.close();
    }
  });
  ...
  test("turns a mid-stream Anthropic transport disconnect into a retryable stream failure after partial events", async () => {
    ...
    try {
      ...
      expect(result.events.map((event) => event.reasoning?.text ?? "").join("")).toContain("271828");
      expect(result.error).toBeInstanceOf(Error);
      expect((result.error as Error).message).toContain("socket connection was closed unexpectedly");
      expect(classifyProviderStreamError(result.error)).toMatchObject({
        code: "provider_stream_error",
        retryable: true,
        fatal: false,
  ...
  test("captures an official Anthropic cache-hit recording without double-counting cached input", async () => {
    ...
    try {
      ...
      expect(mock.requests).toHaveLength(1);
      const captured = mock.requests[0]!;
      await expectCapturedRequestFixture(captured, "anthropic-cache-hit/basic-turn.http.json");
      expect(captured.headers["anthropic-beta"]).toBe("interleaved-thinking-2025-05-14,fine-grained-tool-streaming-2025-05-14");
      expect(captured.body.system).toMatchObject([{ cache_control: { type: "ephemeral" } }]);
      expect(captured.body.messages).toMatchObject([{ content: [{ cache_control: { type: "ephemeral" } }] }]);
      await expectProviderEventsFixture(events, "anthropic-cache-hit/basic-turn.events.json");
      expect(events.at(-1)?.finish).toMatchObject({
...
describe("Session provider golden wire path", () => {
  test("turns a mid-stream Kimi transport disconnect into a retryable stream failure after partial events", async () => {
    ...
    try {
      ...
      expect(result.events.map((event) => event.type)).toContain(ProviderStreamEventType.PROVIDER_STREAM_EVENT_TYPE_REASONING_START);
      expect(result.events.map((event) => event.reasoning?.text ?? "").join("")).toContain("The");
      expect(result.error).toBeInstanceOf(Error);
      expect(classifyProviderStreamError(result.error)).toMatchObject({
        code: "provider_stream_error",
        retryable: true,
  ...
  test("captures a Kimi cache-hit recording from the terminal message_delta usage frame", async () => {
    ...
    try {
      ...
      expect(mock.requests).toHaveLength(1);
      const captured = mock.requests[0]!;
      await expectCapturedRequestFixture(captured, "moonshotai-k3-cache-hit/basic-turn.http.json");
      expect(captured.headers["anthropic-beta"]).toBeUndefined();
      expect(captured.body.system).toMatchObject([{ cache_control: { type: "ephemeral" } }]);
      expect(captured.body.messages).toMatchObject([{ content: [{ cache_control: { type: "ephemeral" } }] }]);
      await expectProviderEventsFixture(events, "moonshotai-k3-cache-hit/basic-turn.events.json");
      expect(events.at(-1)?.finish).toMatchObject({
        reason: ProviderFinishReason.PROVIDER_FINISH_REASON_STOP,
        usage: {
  ...
  test("captures the Z.ai openai-compatible request shape and raises the replayed fixture", async () => {
    ...
    expect(fixture).toContain("recorded 2026-07-04 model_id=zai/glm-5.2 source=real-provider");
    expectFixtureHasNoProviderSecrets(fixture);
    const mock = createMockAnthropicServer(fixture);
    const registry = new ProviderClientRegistry({ fetch: mock.fetch });
    try {
      ...
      expect(captured.body.messages).toEqual([
        {
          role: "system",
          content: "Keep visible output minimal. Do not explain. Use tool calls when requested.",
        },
        {
          role: "user",
          content: "Use brief hidden reasoning to verify 7 + 8 = 15. Then emit visible text exactly: ok. Then call Search exactly once with JSON input {\"query\":\"tetral\"}. Do not add any other visible text.",
        },
      ]);
      expect(captured.body.tools).toEqual([
        {
  ...
  test("carries Z.ai assistant reasoning_content on the openai-compatible wire", async () => {
    ...
    try {
      ...
      expect(mock.requests).toHaveLength(1);
      expect(JSON.stringify(mock.requests[0]?.body)).not.toContain("openaiCompatible");
  ...
  test("carries DeepSeek assistant reasoning_content on the openai-compatible wire", async () => {
    ...
    expect(fixture).toContain("recorded 2026-07-08 model_id=deepseek/deepseek-v4-pro source=real-provider");
    expectFixtureHasNoProviderSecrets(fixture);
    const mock = createMockAnthropicServer(fixture);
    const registry = new ProviderClientRegistry({ fetch: mock.fetch });
    try {
      ...
      expect(mock.requests).toHaveLength(1);
      const captured = mock.requests[0]!;
      await expectCapturedRequestFixture(captured, "deepseek/basic-turn.http.json");
      expect(captured.method).toBe("POST");
      ...
      expect(events.map((event) => event.type)).toContain(ProviderStreamEventType.PROVIDER_STREAM_EVENT_TYPE_REASONING_DELTA);
      expect(events.map((event) => event.text?.text ?? "").join("")).toBe("ok");
      await expectProviderEventsFixture(events, "deepseek/basic-turn.events.json");
      expect(events.at(-1)?.finish).toMatchObject({
        reason: ProviderFinishReason.PROVIDER_FINISH_REASON_STOP,
        usage: {
  ...
  test("turns a mid-stream DeepSeek transport disconnect into a retryable stream failure after partial events", async () => {
    ...
    try {
      ...
      expect(captured.headers.authorization).toBe("Bearer sk-session-deepseek");
      expect(result.events.map((event) => event.type)).toContain(ProviderStreamEventType.PROVIDER_STREAM_EVENT_TYPE_REASONING_START);
      expect(result.events.map((event) => event.reasoning?.text ?? "").join("")).toContain("We");
      expect(result.error).toBeInstanceOf(Error);
      expect(classifyProviderStreamError(result.error)).toMatchObject({
        code: "provider_stream_error",
        retryable: true,
...
function fixtureResponseBody(fixture: string, contentType?: string): string {
  if ((contentType ?? "text/event-stream").startsWith("text/event-stream") && !fixture.endsWith("\n\n")) {
    return `${fixture}\n`;
  }
  return fixture;
}
...
function expectProviderToolTurn(events: readonly ProviderStreamEvent[], options: {
  ...
  const types = events.map((event) => event.type);
  const reasoningStart = types.indexOf(ProviderStreamEventType.PROVIDER_STREAM_EVENT_TYPE_REASONING_START);
  const reasoningEnd = types.indexOf(ProviderStreamEventType.PROVIDER_STREAM_EVENT_TYPE_REASONING_END);
  const textStart = types.indexOf(ProviderStreamEventType.PROVIDER_STREAM_EVENT_TYPE_TEXT_START);
  const textEnd = types.indexOf(ProviderStreamEventType.PROVIDER_STREAM_EVENT_TYPE_TEXT_END);
  const toolInputStart = types.indexOf(ProviderStreamEventType.PROVIDER_STREAM_EVENT_TYPE_TOOL_INPUT_START);
  const toolInputEnd = types.indexOf(ProviderStreamEventType.PROVIDER_STREAM_EVENT_TYPE_TOOL_INPUT_END);
  const toolCallIndex = types.indexOf(ProviderStreamEventType.PROVIDER_STREAM_EVENT_TYPE_TOOL_CALL);
  const finishIndex = types.indexOf(ProviderStreamEventType.PROVIDER_STREAM_EVENT_TYPE_FINISH);
  ...
  expect(events.map((event) => event.text?.text ?? "").join("")).toContain(options.textIncludes);
  expect(JSON.parse(events.map((event) => event.toolInput?.text ?? "").join(""))).toEqual(JSON.parse(options.toolInputJson));
  const toolCalls = events.filter((event) => event.type === ProviderStreamEventType.PROVIDER_STREAM_EVENT_TYPE_TOOL_CALL);
...
async function readGoldenJsonFixture<T extends { readonly _fixtureProvenance?: GoldenFixtureProvenance }>(relativePath: string): Promise<T> {
...
async function collectGoldenJsonFixturePaths(root: URL, prefix = ""): Promise<readonly string[]> {
  ...
  for (const entry of entries) {
    ...
    if (entry.isDirectory()) {
      files.push(...(await collectGoldenJsonFixturePaths(entryURL, relativePath)));
      continue;
    }
...
async function expectCapturedRequestFixture(captured: CapturedAnthropicRequest, relativePath: string): Promise<void> {
  const expected = await readGoldenJsonFixture<GoldenHTTPRequestFixture>(relativePath);
  expect(captured.method).toBe(expected.method);
  expect(captured.pathname).toBe(expected.pathname);
  if (expected.headers !== undefined) {
    expect(redactCapturedHeaders(captured.headers)).toMatchObject(expected.headers);
  }
...
async function expectProviderEventsFixture(events: readonly ProviderStreamEvent[], relativePath: string): Promise<void> {
  const expected = await readGoldenJsonFixture<GoldenEventsFixture>(relativePath);
  const eventTypes = events.map((event) => providerStreamEventTypeToJSON(event.type));
  ...
  if (expected.toolInputJson !== undefined) {
    expect(JSON.parse(events.map((event) => event.toolInput?.text ?? "").join(""))).toEqual(JSON.parse(expected.toolInputJson));
  }
  if (expected.toolCalls !== undefined) {
    expect(events.filter((event) => event.toolCall !== undefined).map((event) => ({
  ...
  expect(providerFinishReasonToJSON(finish?.reason ?? ProviderFinishReason.PROVIDER_FINISH_REASON_UNSPECIFIED)).toBe(expected.finish.reason);
  expect(finish?.usage).toMatchObject(expected.finish.usage);
  if (expected.finish.metadata !== undefined) {
    expect(JSON.parse(finish?.metadataJson ?? "{}")).toEqual(expected.finish.metadata);
  }
}

function eventTypeRuns(eventTypes: readonly string[]): readonly { readonly type: string; readonly count: number }[] {
...
function plainTextAttachment(filename: string, text: string): {
...
function openAIGPT56SolGoldenRequest(sessionId = "sesn_live_gpt56_sol_official_fixture"): ProviderRequest {
  return {
    ...openAIGoldenRequest(),
...
function zaiGoldenRequest(overrides: Partial<ProviderRequest> = {}): ProviderRequest {
  return validProviderRequest({
...
function deepSeekGoldenRequest(overrides: Partial<ProviderRequest> = {}): ProviderRequest {
  return validProviderRequest({
...
async function collectEventsUntilError(events: AsyncIterable<ProviderStreamEvent>): Promise<{
