...
import { RuntimeToolPartState } from "@tetral/gateway-protocol/src/gen/tetral/provider_gateway/v1/provider_gateway.js";
...
const toolParts = firstRequest.messages.flatMap((message) =>
  message.parts.flatMap((part) => part.tool === undefined ? [] : [part.tool])
);
const repair = toolParts[0];
if (
  toolParts.length !== 1 || repair === undefined ||
  repair.state !== RuntimeToolPartState.RUNTIME_TOOL_PART_STATE_ERROR ||
  !firstRequest.messages.some((message) => message.parts.some((part) => part.text?.text === "continuing"))
) {
...
for (const scenario of scenarios) {
  ...
  const requestTools = request.messages.flatMap((message) =>
    message.parts.flatMap((part) => part.tool === undefined ? [] : [part.tool])
  );
  if (requestTools.length !== 1 || JSON.stringify(requestTools[0]) !== JSON.stringify(repair)) {
...
function assertRepairWire(
  ...
  expected: NonNullable<ProviderRequest["messages"][number]["parts"][number]["tool"]>,
) {
  const input = JSON.parse(expected.inputJson) as unknown;
  const error = expected.outputOrErrorJson;
  ...
  if (family === "anthropic") {
    ...
    if (
      ...
      call.part.id !== expected.callId || call.part.name !== expected.name || !jsonEqual(call.part.input, input) ||
      result.part.tool_use_id !== expected.callId || result.part.content !== error || result.part.is_error !== true
  ...
  if (family === "openai") {
    ...
    const result = results[0]!;
    if (
      call.index >= result.index || call.item.call_id !== expected.callId || call.item.name !== expected.name ||
      !jsonEqual(JSON.parse(String(call.item.arguments)), input) || result.item.call_id !== expected.callId ||
  ...
  if (
    call.role !== "assistant" || call.index >= result.index || call.call.id !== expected.callId ||
    fn.name !== expected.name || !jsonEqual(JSON.parse(String(fn.arguments)), input) ||
    result.message.tool_call_id !== expected.callId || result.message.content !== error
...
function summary(
  family: string,
  pathname: string,
  repair: { readonly callId: string; readonly name: string; readonly outputOrErrorJson: string },
  callIndex: number,
  resultIndex: number,
) {
  return {
    ...
    errorMessage: (JSON.parse(repair.outputOrErrorJson) as { error: { message: string } }).error.message,
    callIndex,
    resultIndex,
  };
}
