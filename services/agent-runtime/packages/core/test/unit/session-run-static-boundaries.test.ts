...
const lifecycleForbiddenPatterns: readonly ForbiddenPattern[] = [
	...
	{
		label: "forbidden unloaded thread ensure",
		pattern: /\b(?:ensureRunning|handleEnsureRunning|EnsureRunningResult)\b/,
	},
	{
		label: "forbidden TTL timer",
		pattern: /\bttl\b|\bidle(?:Timer|Timeout|Fiber)\b/i,
	},
	{
		label: "forbidden cleanup timer",
		pattern: /\bcleanup(?:Timer|Timeout|Fiber)\b/i,
	},
	{
		label: "forbidden payload ingress",
		pattern: /\b(?:eventBody|providerRequest|runtimeMessage|modelInfo)\b/,
	},
	{
		label: "forbidden HTTP/RPC transport",
		pattern:
			/\b(?:http|rpc)(?:Handler|Server|Adapter|Transport|Route|Endpoint)?\b/i,
	},
...
async function collectLifecycleSourcePaths(): Promise<readonly string[]> {
	const files = (
		await Promise.all(
			lifecycleLayerRoots.map((root) => collectTypeScriptFiles(root)),
		)
	).flat();
	return files.sort((left, right) =>
		packageRelativePath(left).localeCompare(packageRelativePath(right)),
	);
}
...
function collectLifecycleBoundaryViolations(
	relativePath: string,
	text: string,
): readonly string[] {
	...
	for (const importSpecifier of importSpecifiers(text)) {
		const allowedThreadLoopRuntimeImport =
			(relativePath === "src/thread-loop/thread-loop.ts" &&
				(importSpecifier === "../runtime/context-projection.js" ||
		...
		if (importSpecifier.startsWith(".") && extname(importSpecifier) !== ".js") {
			violations.push(
				`${relativePath}: relative import without .js suffix (${importSpecifier})`,
			);
		}
		if (
			!allowedThreadLoopRuntimeImport &&
			!allowedSessionManagerRuntimeImport &&
			/^(?:.*\/)?(?:providers|runtime)\//.test(importSpecifier)
		) {
			violations.push(
				`${relativePath}: guarded runtime/provider import (${importSpecifier})`,
			);
		}
	}
	return violations;
...
describe("session run static boundaries", () => {
	test("lifecycle roots expose the new Agent Pod ownership files and no removed service/state files", async () => {
		...
		expect(
			await pathExists(
				sourceUrl("src/session-run-service/session-run-service.ts"),
			),
		).toBe(false);
		expect(
			await pathExists(sourceUrl("src/session/session-run-state.ts")),
		).toBe(false);
		expect(
			await pathExists(sourceUrl("test/unit/session-run-service.test.ts")),
		).toBe(false);
		expect(
			await pathExists(sourceUrl("test/unit/session-run-state.test.ts")),
		).toBe(false);
	});
	...
	test("lifecycle sources reject legacy names, payload ingress, timers, transports, and unsafe primitives", async () => {
		...
		for (const [relativePath, text] of sources) {
			violations.push(
				...collectLifecycleBoundaryViolations(relativePath, text),
			);
		}
	...
	test("boundary scanner rejects realistic forbidden variants", () => {
		...
		expect(violations).toContain(
			"src/session/bad.ts: relative import without .js suffix (../runtime/stream-service)",
		);
		expect(violations).toContain(
			"src/session/bad.ts: guarded runtime/provider import (../runtime/stream-service)",
		);
		expect(violations).toContain(
			"src/session/bad.ts: forbidden SessionRunService",
		);
		expect(violations).toContain(
			"src/session/bad.ts: forbidden SessionRunState",
		);
		...
		expect(violations).toContain(
			"src/session/bad.ts: forbidden payload ingress",
		);
		expect(violations).toContain(
			"src/session/bad.ts: forbidden Bun.serve transport",
		);
	});
	...
	test("string-keyed maps are limited to residency, reviewer memo, and pure checkpoint extraction", async () => {
		...
		expect(normalizedSessionEntry).toBe(
			'workspaceId:string;readonlysessionId:string;bindingId:string;bindingGeneration:number;readonlythreads:Map<string,ThreadEntry>;readonlytoolCoordinator:SessionToolCoordinator;readonlyruntimeShutdown:RuntimeShutdownObservation;readonlycontrolGate:Semaphore.Semaphore;readonlyconfiguration:SessionConfiguration;sharedStateStatus:"initializing"|"ready"|"failed";readonlysharedStateInitializerThreadId:string;readonlysharedStateReady:Promise<boolean>;readonlycompleteSharedStateReady:(ready:boolean)=>void;',
		);
		expect(normalizedManager).toContain(
			"toolCoordinator:newSessionToolCoordinator({",
		);
		expect(normalizedManager).toContain(
			"maxConcurrentTools:options.maxConcurrentTools??8,",
		);
		expect(normalizedManager).toContain(
			"newThreadRuntime.ThreadRuntime(identity,approvalReviewer,toolCoordinator,sessionConfiguration,)",
		);
		expect(normalizedManager).toContain(
			"constsessions=newMap<string,SessionEntry>();",
		);
		expect(normalizedManager).toContain(
			"constsessionKey=(workspaceId:string,sessionId:string):string=>`${workspaceId}\\u0000${sessionId}`;",
		);
		expect(normalizedManager).toContain(
			"constcommandSessionKey=(command:{readonlyworkspaceId:string;readonlysessionId:string;}):string=>sessionKey(command.workspaceId,command.sessionId);",
		);
		expect(normalizedManager).toContain(
			"constawaitRunSlot=(runSlot:ThreadRunSlot,):Effect.Effect<Exit.Exit<ThreadLoop.ThreadLoopRunResult,unknown>>=>Deferred.await(runSlot.doneDeferred).pipe(Effect.exit)",
		);
		expect(normalizedManager).toContain("construnScope=yield*Scope.make();");
		expect(normalizedManager).toContain(
			"constfiber=yield*Effect.forkIn(run,runScope);",
		);
		expect(normalizedManager).toContain(
			"constcontrolIdentity=(command:RuntimeThreadAddressState,):ThreadRuntime.RuntimeThreadIdentity=>({",
		);
		expect(normalizedManager).toContain(
			"workspaceId:command.workspaceId,sessionId:command.sessionId,sessionThreadId:command.sessionThreadId,bindingId:command.bindingId,bindingGeneration:command.bindingGeneration,",
		);
		expect(normalizedManager).toContain(
			"constsessionEntry=sessions.get(commandSessionKey(command));if(sessionEntry===undefined){return{ok:true,sessionId,created:false,applied:false,noResidency:true,};}",
		);
		expect(normalizedManager).toContain(
			'sessionEntry.sharedStateStatus!=="ready"||[...sessionEntry.threads.values()].some((threadEntry)=>threadEntry.installationState!=="ready"||threadEntry.runSlot!==undefined,)',
		);
		expect(normalizedManager).toContain(
		...
		expect(normalizedManager).not.toContain(
			'constthreadResult=getOrCreateThreadEntry(controlIdentity(command));if(threadResult===undefined){return{ok:false,sessionId,reason:"local_session_capacity_exceeded"};}if(threadResult.threadEntry.runSlot!==undefined)',
		);
		expect(normalizedState).not.toMatch(/pending(?:Wake|Notify)/);
		expect(normalizedManager.match(/setTimeout/g) ?? []).toHaveLength(1);
		expect(normalizedManager).toContain(
			'consttimeout=setTimeout(()=>resolve(true),durationMs);signal.addEventListener("abort",()=>{clearTimeout(timeout);resolve(false);},{once:true},);',
		);
	...
	test("production source has no durable active-run or run-status ownership", async () => {
		...
		const blockedPatterns: readonly ForbiddenPattern[] = [
			...
			{
				label: "SQL table",
				pattern: /\b(?:active_runs|run_statuses|CREATE\s+TABLE)\b/i,
			},
			{
				label: "database client import",
				pattern:
					/\bfrom\s+["'](?:pg|postgres|postgres-js|sqlite|better-sqlite3|bun:sqlite|drizzle|kysely|prisma)["']/,
			},
		] as const;
		...
		for (const filePath of sourceFiles) {
			...
			for (const blockedPattern of blockedPatterns) {
				...
				if (
					blockedPattern.label === "SessionStatus" &&
					relativePath === "src/contracts/runtime.ts"
				) {
					continue;
				}
				violations.push(
					`${relativePath}: durable run-state term ${blockedPattern.label}`,
				);
			}
		}
	...
	test("cold preload publishes installing residency before the shared-state gate and pending-tool hydration", async () => {
		...
		const threadLoopSource = await readFile(
			sourceUrl("src/thread-loop/thread-loop.ts"),
			"utf8",
		);
		...
		const pendingAssignmentIndex = normalizedPreload.indexOf(
			"constpendingToolUseInstall",
		);
		const residencyIndex = normalizedPreload.indexOf(
			'getOrCreateThreadEntry(identity,metadata,"installing",)',
		);
		const sharedStateGateIndex = normalizedPreload.indexOf(
		...
		expect(
			normalizedPreload.indexOf("completeSharedStateReady(true)"),
	...
	test("pending approval hot state stores narrow settlement descriptors, never SessionProcessor", async () => {
		...
		const stateStart = stateSource.indexOf(
		...
		const normalizedPendingState = normalizeSource(pendingState);
