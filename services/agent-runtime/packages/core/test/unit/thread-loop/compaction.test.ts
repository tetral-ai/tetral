...
import type { RuntimeDependencies, SessionEvent, SessionEventWriter, SessionEventWriterRequestEndEnvelope, SessionEventWriterRuntimeTerminationEnvelope } from "../../../src/contracts/runtime.js";
    ...
    const llm = queuedLLMService([
        [
            ...
            { type: "text-end", id: "review-first" },
            {
                type: "finish",
                finishReason: "stop",
                usage: { inputTokens: 96000, outputTokens: 1, reasoningTokens: 0, cacheReadTokens: 0, cacheWriteTokens: 0 },
...
test("compaction boundary uses zero only for an empty own-message list", () => {
    ...
    expect(compactionBoundaryMessageSequence([
...
test("compaction updates the latest checkpoint and carries its legacy recent block as opaque context", async () => {
    ...
    const previousCheckpoint = RuntimeMessageSchema.parse({
        ...userMessage("checkpoint-previous", 0, `<conversation-checkpoint>
...
{"legacy":"recent"}
    ...
    let pendingBeforeAgentRequest: readonly string[] | undefined;
    const writerBase = writerFrom((envelope) => {
        if (envelope.event.type === "span.model_request_start") {
            requestStartCount += 1;
            if (requestStartCount === 2) {
                pendingBeforeAgentRequest = session.state.threadTurnReduction().checkpoint.pendingInputMessageIds;
            }
        }
        return {
            ok: true,
    ...
    const result = await Effect.runPromise(Effect.gen(function* () {
        const threadLoop = yield* ThreadLoop.Service;
        return yield* threadLoop.run(session, testRunCustody());
    }).pipe(Effect.provide(runtimeThreadLoopLayer(new RecordingContextLoader([previousCheckpoint], { type: "messages", messages: [userMessage("user-fresh", 1, "fresh continuation")] }), {
        compaction: {},
...
test("direct Effect interruption closes an ACKed compaction request before interruption finishes", async () => {
    ...
    try {
        await waitForCondition(() => active.observedAbortSignal?.aborted === true || interruptFinished, "compaction abort or interrupt completion");
        expect(active.session.state.runtimeShutdownRequested()).toBe(false);
        expect(active.observedAbortSignal?.aborted).toBe(true);
        await waitForCondition(() => active.requestEndEnvelopes.length === 1 || interruptFinished, "compaction request-end write or interrupt completion");
...
test("compaction cancellation reports event_write_failed when request-end is not ACKed", async () => {
    ...
    try {
        await waitForCondition(() => active.observedAbortSignal?.aborted === true, "compaction shutdown abort");
        expect(active.observedAbortSignal?.aborted).toBe(true);
        await waitForCondition(() => active.requestEndEnvelopes.length === 1, "compaction request-end write");
        expect(active.requestEndEnvelopes).toHaveLength(1);
        const requestEnd = active.requestEndEnvelopes[0];
        if (requestEnd === undefined) {
            throw new Error("missing compaction request end");
...
test("compaction fit refusal stops before provider work when serialized history exceeds the model window", async () => {
    ...
    const loader = new RecordingContextLoader([userMessage("user-oversized", 0, compactionTransportHistory("oversized history"))], { type: "messages", messages: [userMessage("user-new", 1, "new request")] });
    const requests: LLMRequest[] = [];
    const appended: SessionEvent[] = [];
    const result = await Effect.runPromise(Effect.gen(function* () {
        const threadLoop = yield* ThreadLoop.Service;
...
test("compaction hot apply preserves messages ACKed after the compaction snapshot", async () => {
    ...
    const loader = new RecordingContextLoader([userMessage("user-old", 0, compactionHistory("old context that should be summarized"))], { type: "messages", messages: [userMessage("user-new", 1, "new request")] });
    const requests: LLMRequest[] = [];
...
test("task notification arriving during reactive compaction joins the preserved agent request", async () => {
    ...
    const layer = runtimeThreadLoopLayer(loader, { llmService: llm, writer, compaction: {} });
...
test("runtime layer retries failed compaction before normal provider request", async () => {
    ...
    expect(requests.slice(0, 2).map((request) => request.limits?.maxOutputTokens)).toEqual([4096, 4096]);
...
test("a stale compaction Request Start receipt discards hot state before provider dispatch", async () => {
    ...
    const loader = new RecordingContextLoader([], {
        type: "messages",
        messages: [userMessage("user-stale-compaction", 0, compactionHistory("compact stale"))],
    });
    const baseWriter = writerFrom((envelope) => ({
        ok: true,
        writeId: envelope.writeId,
