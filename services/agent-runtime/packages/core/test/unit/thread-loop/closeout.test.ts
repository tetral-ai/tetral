...
describe("ThreadLoop", () => {
	...
	test("a late requires-action ACK retains the unsettled Tool route", async () => {
		...
		session.state.installThreadTurn(
			{
				...
				request: {
					...
					requestStartEventId: "sevt_partial_requires_action_start",
					requestKind: "agent_provider_request",
					contextThroughMessageSequence: 0,
					requestEnd: {
					...
					toolMembers: [
						{
							memberKind: "public_tool_use",
							modelToolCallId: "call_partial_requires_action_settled",
							toolUseEventId: settledToolUseEventId,
							toolName: "Write",
						},
						{
							memberKind: "public_tool_use",
							modelToolCallId: "call_partial_requires_action_pending",
		...
		const writer: SessionEventWriter = {
			...writerFrom(() => ({
				ok: true,
				eventId: "sevt_partial_requires_action_event",
				type: "committed",
			})),
			finishIdle: async (envelope) => {
				finishIdleStarted.resolve();
