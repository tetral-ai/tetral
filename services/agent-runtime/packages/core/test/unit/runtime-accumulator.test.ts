...
const source = {
	providerId: "openai" as RuntimeProcessorSource["providerId"],
	modelId: "model",
};
...
function deferredEvent(): {
	...
	const promise = new Promise<SessionEventWriterAppendEvent>((settle) => {
		resolve = settle;
	});
...
function member(
	toolCallId: string,
	event: Promise<SessionEventWriterAppendEvent>,
): FrozenAssistantPartAppend {
	return {
		source,
		toolCallId,
		event,
...
describe("request Assistant member sequencing", () => {
	test("a later authorized Tool cannot overtake an earlier permission wait", async () => {
		...
		const firstCommit = sequencer.enqueue(
			member("call_first", first.promise),
		).committed;
		const secondCommit = sequencer.enqueue(
			member("call_second", second.promise),
		).committed;
		second.resolve({
			type: "agent.tool_use",
			name: "exec_command",
