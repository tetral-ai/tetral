...
function isProviderMetadataJson(value: unknown): value is ProviderMetadataJson {
	...
	return Object.entries(value).every(
		([key, item]) => hasOnlyUnicodeScalars(key) && isProviderMetadataJson(item),
	);
}
...
function isProviderMetadata(value: unknown): value is ProviderMetadata {
	return (
		...
		Object.entries(value).every(
			([key, item]) =>
				hasOnlyUnicodeScalars(key) && isProviderMetadataJson(item),
		)
	);
}
...
export const ProviderMetadataSchema = z.custom<ProviderMetadata>(
	isProviderMetadata,
	"ProviderMetadata",
);
...
export function normalizeProviderError(
	...
	return ProviderErrorSchema.parse({
		...
		...(safeOptionalIdentifier(input.providerId) !== undefined
			? { providerId: safeOptionalIdentifier(input.providerId) }
			: {}),
		...(safeOptionalIdentifier(input.modelId) !== undefined
			? { modelId: safeOptionalIdentifier(input.modelId) }
			: {}),
		...(safeOptionalNonNegativeInteger(input.statusCode) !== undefined
			? { statusCode: safeOptionalNonNegativeInteger(input.statusCode) }
			: {}),
		...(safeOptionalNonNegativeInteger(input.retryAfterMs) !== undefined
			? { retryAfterMs: safeOptionalNonNegativeInteger(input.retryAfterMs) }
			: {}),
...
function providerErrorCode(value: string | undefined): ProviderErrorCode {
	return ProviderErrorCodes.includes(value as ProviderErrorCode)
		? (value as ProviderErrorCode)
		: "provider_unknown";
}

function boundedProviderErrorMessage(
