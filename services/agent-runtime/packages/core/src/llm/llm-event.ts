...
const RuntimePersistenceErrorCodes = [
	"unavailable",
	"timeout",
	"conflict",
...
const SanitizedTextSchema = z
	.string()
	.refine(
		(value) => isWithinUtf8ByteBudget(value, MaxProviderErrorMessageBytes),
		`text must be at most ${MaxProviderErrorMessageBytes} UTF-8 bytes`,
	)
	.transform(sanitizeRuntimeText);
const SanitizedIdentifierSchema = IdentifierSchema.refine(
...
const RuntimePreviewTextSchema = z
	.string()
	.refine(
		(value) => isWithinUtf8ByteBudget(value, RuntimePreviewTextMaxBytes),
		`preview must be at most ${RuntimePreviewTextMaxBytes} UTF-8 bytes`,
	);
const RuntimeIdentifierSchema = IdentifierSchema.refine(
	(value) => isWithinUtf8ByteBudget(value, MaxIdBytes),
	`identifier must be at most ${MaxIdBytes} UTF-8 bytes`,
);
const ProviderMetadataSchema = RawProviderMetadataSchema.refine(
	(metadata) =>
		serializedRuntimeJsonByteLength(metadata as RuntimeJsonValue) <=
		MaxMetadataBytes,
...
export const RuntimeUsageSchema = z.strictObject({
	...
	providerUsageJson: z
		.string()
		.refine(
			(value) => isWithinUtf8ByteBudget(value, MaxProviderUsageJsonBytes),
			`provider usage must be at most ${MaxProviderUsageJsonBytes} UTF-8 bytes`,
		)
		.optional(),
});
...
const RuntimeToolInputSchema = RuntimeJsonValueSchema.refine(
	(value) =>
		serializedRuntimeJsonByteLength(value) <= MaxProviderToolCallInputJsonBytes,
...
export type RuntimeAttachmentRejection = z.infer<
	typeof RuntimeAttachmentRejectionSchema
>;
...
export function runtimeFailureFromProviderError(
	error: ProviderError,
	retryStatus?: RuntimeFailure["retryStatus"],
): RuntimeFailure {
