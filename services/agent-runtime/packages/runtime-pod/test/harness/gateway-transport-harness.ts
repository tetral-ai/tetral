...
import { estimatedRuntimeContextTokens } from "../../../core/src/thread-loop/compaction.js";
...
const RequiredHeadroomRatio = 0.2;
...
export async function runGatewayCapacityProof(): Promise<
	readonly GatewayCapacityMeasurement[]
> {
			...
			async (input: RequestInfo | URL, init?: RequestInit) => {
				...
				for (const marker of activeMarkers) {
					if (!body.includes(marker)) {
						throw new Error(
							`capacity proof provider wire dropped marker ${marker}`,
						);
					}
				}
				return new Response(
					JSON.stringify({
						error: {
							type: "server_error",
							message: "synthetic capacity response",
						},
					}),
					{
						status: 500,
		...
		providerStreamer: {
			stream: async function* (input) {
				receivedBytes.set(
					...
					ProviderRequestMessage.encode(input.request).finish().byteLength,
				);
	...
	try {
		...
		for (const vector of vectors) {
			...
			for (const route of CapacityRoutes) {
				...
				if (events.length === 0) {
					throw new Error(
						"capacity proof transport returned no terminal provider event",
					);
				}
				...
				if (received === undefined || wire === undefined) {
					throw new Error(
						"capacity proof did not cross the production Gateway and provider wire",
					);
				}
				encodedRequestBytes = Math.max(
					encodedRequestBytes,
			...
			measurements.push({
				...
				headroomRatio:
					(MaxGatewayRequestGrpcMessageBytes - encodedRequestBytes) /
					MaxGatewayRequestGrpcMessageBytes,
				loweredBytesByFamily,
				...(vector.loadedContextBytes === undefined
					? {}
					: { loadedContextBytes: vector.loadedContextBytes }),
			});
		}
		return measurements;
	} finally {
...
export async function runGatewayCapacityFuseMutation(): Promise<void> {
	...
	const server = createGatewayGrpcServer(
		new ProviderGatewayServiceShell({
			authenticator: {
				authenticate: async () => ({
					ok: true,
					serviceAccount: {
			...
			logger: { info: () => undefined, error: () => undefined },
			providerStreamer: {
				stream: (input) => successfulCapacityStream(input.request),
			},
			credentialResolver: capacityCredentialResolver(),
		}),
	);
...
export async function runGatewayReceiveCapacityFuseMutation(): Promise<void> {
	...
	const implementation: ProviderGatewayServiceServer = {
		streamProviderRequest(call) {
			void (async () => {
				try {
					for await (const event of service.streamProviderRequest(
						call.request,
						call.metadata,
					)) {
						call.write(event);
					...
					call.emit(
						...
						error instanceof Error
							? error
							: new Error("capacity mutation failed"),
					);
				}
			})();
		},
...
export async function runLargeToolInputMappingProof() {
	...
	const base = assembledVector("large_tool_input", [
		textMessage("large_tool_prompt", 1, "write it"),
	]);
...
export async function runRecordedGLMTransportProof() {
	...
	const base = assembledVector("glm_recorded_stream", [
		textMessage("glm_recorded_prompt", 1, "use Search"),
	]);
		...
		authenticator: {
			authenticate: async () => ({
				...
				serviceAccount: {
					namespace: "tetral",
					name: "agent-runtime",
					podUid: "pod_glm_recorded_stream",
				},
			}),
		},
		runtimeBindingTokenVerifier: { verify: () => true },
...
export async function runMaximumReadTransportProof() {
	...
	const assembled = assembledVector(
		"maximum_read",
		[contextEntry(1, "assistant", runtimeParts)],
		undefined,
	...
	const service = new ProviderGatewayServiceShell({
		...
		logger: { info: () => undefined, error: () => undefined },
		providerStreamer: {
			stream: (providerInput) => providerRegistry.stream(providerInput),
		},
		credentialResolver: capacityCredentialResolver(),
	});
	const server = createGatewayGrpcServer(service);
	...
	return {
		...
		providerBodyContainsMarkers:
			providerBody.includes("TETRAL_MAXIMUM_READ_HEAD") &&
			providerBody.includes("TETRAL_MAXIMUM_READ_TAIL"),
		projectedOutputBytes: Buffer.byteLength(
...
export async function runGatewayReceiveFuseProof() {
	const base = assembledVector("receive_fuse", [
		textMessage("receive_fuse_prompt", 1, "test"),
	]);
...
export async function runPreEventGatewayFailureProof() {
	const base = assembledVector("pre_event_failure", [
		textMessage("pre_event_prompt", 1, "test"),
	]);
	const validRequest = requestForRoute(base.request, CapacityRoutes[0]);
	return {
		invalidArgument: await collectPreEventGatewayFailure(
...
export async function runGatewayAbsentRetryDelayProof() {
	const base = assembledVector("absent_retry_delay", [
		textMessage("absent_retry_delay_prompt", 1, "test"),
	]);
	const request = requestForRoute(base.request, CapacityRoutes[0]);
	const service = new ProviderGatewayServiceShell({
		authenticator: {
	...
	try {
		...
		if (terminal?.type !== "provider-error") {
			throw new Error(
				"absent retry delay proof did not receive provider error",
			);
		}
...
async function collectPreEventGatewayFailure(
	...
	try {
		return await Effect.runPromise(
			Stream.runCollect(createLLMService(client).stream(request)).pipe(
				Effect.flip,
			),
		);
	} finally {
...
function requestForRoute(
	...
	return {
		...
		tools:
			route.family === "openai"
				? request.tools
				: request.tools.filter((tool) => tool.freeform === undefined),
	};
}
...
function capacityCredentialResolver(): ProviderCredentialResolver {
	return {
		resolve: async (request: Pick<ProviderRequest, "model">) => {
			const route = CapacityRoutes.find(
				(candidate) =>
					candidate.providerId === request.model?.providerId &&
					candidate.modelId === request.model.modelId,
			);
			if (route === undefined) {
...
async function capacityVectors(): Promise<
	readonly {
...
> {
	const ordinaryMarkers = [
		"TETRAL_ORDINARY_FIRST",
		"TETRAL_ORDINARY_LAST",
	] as const;
	const ordinary = fitMessagesToTokenTarget((budget) => [
		textMessage(
			"ordinary_ascii",
			1,
			markedText(Math.floor(budget / 2), ordinaryMarkers[0], "", "x"),
		),
		textMessage(
			"ordinary_utf8",
			2,
			markedText(budget - Math.floor(budget / 2), "", ordinaryMarkers[1], "界"),
		),
	]);
	const toolParts = completedToolParts();
	const toolMarkers = [
		"TETRAL_TOOL_INPUT_FIRST",
		"TETRAL_TOOL_INPUT_LAST",
		"TETRAL_TOOL_OUTPUT_FIRST",
		"TETRAL_TOOL_OUTPUT_LAST",
	] as const;
	const withTools = fitMessagesToTokenTarget((budget) => [
		textMessage("tool_context", 1, "x".repeat(budget)),
		contextEntry(2, "assistant", toolParts),
	]);
	const mcpMarkers = ["TETRAL_MCP_FIRST", "TETRAL_MCP_LAST"] as const;
	const withCatalog = fitMessagesToTokenTarget((budget) => [
		textMessage("catalog_context", 1, "x".repeat(budget)),
	]);
	const fragmentMarkers = [
		"TETRAL_FRAGMENT_FIRST",
		"TETRAL_FRAGMENT_LAST",
	] as const;
	...
	const escapeDenseMarkers = [
		"TETRAL_ESCAPE_OUTPUT_FIRST",
		"TETRAL_ESCAPE_OUTPUT_LAST",
	] as const;
	const escapeDenseToolMessages = Array.from({ length: 48 }, (_, index) =>
		contextEntry(
			index + 1,
			"assistant",
			escapeDenseToolParts(index, 48, escapeDenseMarkers),
		),
	);
	const escapeDenseOutputs = fitMessagesToTokenTarget((budget) => [
		...
		textMessage(
			"escape_dense_filler",
			escapeDenseToolMessages.length + 1,
			"x".repeat(budget),
		),
	]);
	...
	return [
		assembledVector(
			"ordinary_ascii_and_utf8",
			ordinary,
			undefined,
			ordinaryMarkers,
		),
		assembledVector(
			"escaped_tool_input_and_max_output",
			withTools,
			undefined,
			toolMarkers,
		),
...
function assembledVector(
	name: string,
	runtimeContext: readonly RuntimeContextEntry[],
	toolCatalog: ToolCatalog | undefined = undefined,
	wireMarkers: readonly string[] = [],
) {
	const projected = toGatewayProviderContext(runtimeContext);
	if (!projected.ok) {
	...
	if (!assembled.ok) {
		throw new Error(
			`capacity vector ${name} did not assemble: ${assembled.error.code}: ${assembled.error.message}`,
		);
	}
	return {
		name,
...
function fitMessagesToTokenTarget(
	build: (characterBudget: number) => readonly RuntimeContextEntry[],
): readonly RuntimeContextEntry[] {
	let budget = CapacityTargetTokens * 4;
	let messages = build(budget);
	for (let attempt = 0; attempt < 4; attempt += 1) {
		const estimated = estimatedRuntimeContextTokens(messages);
...
function completedToolParts(): readonly RuntimeContextPart[] {
	...
	const outputLength =
		512 * 1024 - Buffer.byteLength(JSON.stringify({ text: "" }), "utf8");
...
function escapeDenseToolParts(
	index: number,
	count: number,
	markers: readonly [string, string],
): readonly RuntimeContextPart[] {
	const output = markedText(
		80 * 1024,
		index === 0 ? markers[0] : "",
...
function maximumReadEnvelope(): string {
	...
	if (Buffer.byteLength(result, "utf8") !== 200_000) {
		throw new Error(
			"maximum Read envelope does not match the 200000-byte contract",
		);
	}
	return result;
}
...
async function collectGatewayEvents(
	...
	if (completion.outcome !== "eof") {
		throw new Error(
			`Gateway stream did not complete normally: ${completion.outcome}`,
		);
	}
	return events;
}
...
function textMessage(
	...
	return contextEntry(sequence, "user", [{ type: "text", text }]);
}

function contextEntry(
	messageSequence: number,
	contextKind: "user" | "assistant",
	parts: readonly RuntimeContextPart[],
): RuntimeContextEntry {
	return { messageSequence, contextKind, parts: [...parts] };
}

export function capacityProofHasRequiredHeadroom(
	measurement: GatewayCapacityMeasurement,
): boolean {
	return (
