...
const committedInput = new Promise<unknown>((resolve) => { resolveCommitResult = resolve; });
const generatedBridgeClient = input.bridgeAddress === undefined
  ? undefined
  : new AgentRuntimeBridgeServiceClient(input.bridgeAddress, credentials.createInsecure());
const bridgeLoader = new BridgeAPIContextLoader({
  address: input.bridgeAddress ?? "unused.test",
  tokenPath: "/unused/token",
  metadataFactory: async () => new Metadata(),
  client: generatedBridgeClient ?? {
    loadContext: (
      _request: unknown,
      _metadata: Metadata,
    ...
    commitTaskNotificationResult: (
      ...
      callback(null, input.commitResponse ?? {
        rejected: { reason: TaskNotificationRejectionReason.TASK_NOTIFICATION_REJECTION_REASON_DURABLE_RESULT_INVALID },
      });
      return { cancel: () => undefined };
    },
...
const contextLoader = {
  loadThreadContext: input.bridgeAddress === undefined
  ...
  commitAcceptedInput: async (...args: Parameters<typeof bridgeLoader.commitAcceptedInput>) => {
    const result = await bridgeLoader.commitAcceptedInput(...args);
    resolveCommitResult?.(result);
    return result;
  },
  refreshRuntimeBindingToken: bridgeLoader.refreshRuntimeBindingToken.bind(bridgeLoader),
};
let nextID = 0;
...
const client = new AgentRuntimePodServiceClient(`127.0.0.1:${port}`, credentials.createInsecure());
try {
  if (input.bridgeAddress === undefined) {
    const response = await new Promise<{ readonly accepted?: unknown; readonly duplicate?: unknown }>((resolve, reject) => {
      ...
      }, new Metadata(), (error, value) => error === null ? resolve(value) : reject(error));
    });
    if (response.accepted === undefined && response.duplicate === undefined) {
      throw new Error(`Runtime did not accept the task notification through generated gRPC: ${JSON.stringify(response)}`);
    }
  }
  const commitResult = await committedInput;
  if (declaration === undefined && input.bridgeAddress === undefined) {
    throw new Error("resident ThreadLoop did not cross the Bridge declaration adapter");
  }
  process.stdout.write(JSON.stringify({ ...(declaration !== undefined ? { declaration } : {}), commitResult }));
} finally {
  client.close();
