...
function request(
	...
	const catalog = createToolCatalog({
		family: toolName === "exec_command" ? "gpt" : "claude",
	});
...
const commandResult = await runner.awaitSandboxExecution(
	request("exec_command", { cmd: "true" }),
);
const fileResult = await runner.awaitSandboxExecution(
	request("Write", { content: "ok", file_path: "result.txt" }),
);
...
if (
	...
	throw new Error(
		"Runtime Bridge adapter did not declare the Sandbox Tool error",
	);
}
console.log(
	JSON.stringify({
