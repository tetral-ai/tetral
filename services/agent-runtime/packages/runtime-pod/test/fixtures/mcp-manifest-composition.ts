...
async function proveNextProviderRejectsStaleTool(
	...
	readonly mcpConnectorCalls: number;
		...
		entries: [
			{
				messageSequence: 1,
				contextKind: "user",
				parts: [{ type: "text", text: "use the removed tool" }],
			},
		],
	});
		...
		Effect.gen(function* () {
			...
			Effect.provide(
				runtimeThreadLoopLayer(loader, {
					...
					onStream: (request) =>
						providerToolNames.push(request.tools.map((tool) => tool.name)),
...
async function applyWarmTransition(input: CompositionInput): Promise<{
	...
	try {
		...
		const commandResponse = await service.applyRuntimeConfig(
		...
		const staleResponse = await service.applyRuntimeConfig(
			{
				...
				mcpManifest: {
					mcpServerName:
						input.runtimeCommandRequest.mcpManifest?.mcpServerName ?? "github",
		...
		if (staleResponse.duplicate === undefined) {
			throw new Error(
				"warm runtime command host rejected the stale replay envelope",
			);
		}
	...
	try {
		...
		const preload = await Effect.runPromise(
			manager.preloadThread({
				...
				...(loaded.runtimeConfigPatch !== undefined
					? { runtimeConfigPatch: loaded.runtimeConfigPatch }
					: {}),
				...(loaded.mcpManifests !== undefined
					? { mcpManifests: loaded.mcpManifests }
					: {}),
			}),
		);
		if (!preload.ok || !preload.applied) {
...
async function buildManager(): Promise<{
	...
	const managerLayer = SessionManager.layer({
		...
		resolveMCPManifestEligibility: (patches, serverName) => {
			...
			catalogs.push(
				runtimeToolPolicyForThread(undefined, payloads, "claude").toolCatalog,
			);
			return runtimeMCPManifestEligibilityFromPatchPayloads(
				payloads,
				serverName,
			);
		},
...
function runtimeControlService(
	...
	const runHost = {
		handleRuntimeConfigPatch: async (
			sessionId: string,
			command: Parameters<
				SessionManager.Interface["applyRuntimeConfigPatch"]
			>[1],
		) =>
			await Effect.runPromise(
				manager.applyRuntimeConfigPatch(sessionId, command),
			),
...
function assertCatalogState(
	...
	if (
		...
		event.toolCatalogEligible !== expectedPresent
	) {
		throw new Error(
