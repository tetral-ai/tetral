...
const writer: SessionEventWriter = {
	settleToolResult: async (envelope) =>
		await productionWriter.settleToolResult(envelope),
	...
	writeRequestEnd: async (envelope) =>
		await productionWriter.writeRequestEnd(envelope),
	...
	commitRuntimeTermination: async (envelope) =>
		await productionWriter.commitRuntimeTermination(envelope),
};

class RecordingBridgeRepairStore extends RuntimeInternalToolRepairStore {
	readonly repairs: RuntimeInternalToolRepairCommit[] = [];
	private readonly committer = new BridgeAPIInternalToolRepairCommitter(
		bridgeOptions,
	);
		...
		Effect.provide(
			runtimeThreadLoopLayer(
				...
				}),
				{
						...
						[
							[
								...
								{ type: "text-end", id: "text-before-invalid-tool" },
								{
									...
									inputPreview: {
										preview: '{"command":"must not execute"}',
										truncated: false,
									},
								},
					...
					providerCallRuntime: {
						systemInstructions: "invalid-tool repair fixture",
					},
					runtimePolicy: () => ({
					...
					awaitSandboxExecution: () => {
						...
						return {
							type: "completed",
							output: { text: "must not await", truncated: false },
						};
					},
				},
			),
...
if (
	...
	repair === undefined ||
	store.repairs.length !== 1
) {
	bridgeClient.close();
	throw new Error(
		`Runtime invalid-tool fixture did not produce one completed repair: ${JSON.stringify({ result, repairs: store.repairs })}`,
	);
}
