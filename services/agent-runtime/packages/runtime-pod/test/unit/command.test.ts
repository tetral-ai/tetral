...
describe("Runtime Pod command entrypoint", () => {
	test("process env projection ignores unrelated inherited variables", () => {
		...
		if (config.ok) {
			...
			expect(JSON.stringify(config.config)).not.toContain(
				"postgres://must-not-be-read",
			);
	...
	test("production dependency assembly feeds one stream timeout to ordinary and compaction requests", async () => {
		...
		try {
			expect(capturedOptions?.threadLoop.providerCallRuntime?.timeoutMs).toBe(
				765_432,
			);
	...
	test("production dependency assembly routes closeout records to metrics and severity-aware logging", async () => {
		...
		try {
			...
			expect(dependencies.metrics.snapshot().closeoutEvents).toEqual(
				new Map([
	...
	test("production manifest observability reports effective eligibility for applied and stale generations", async () => {
				...
				contentJson: JSON.stringify({
					mcp_manifest: {
						...
						tools: [
							{
								name: "search",
								description: "CANARY_MANIFEST_SECRET",
		...
		try {
			const eligible =
				capturedOptions?.resolveMCPManifestEligibility?.(
					effectivePatches,
					"github",
				) ?? false;
			const activeServerWithoutToolset =
				capturedOptions?.resolveMCPManifestEligibility?.(
					[
						{
							generation: 4,
			...
			capturedOptions?.recordMCPManifestUpdate?.({
				workspaceId: "default",
				sessionId: "sesn_manifest_log",
				mcpServerName: "github",
				disposition: "applied",
				source: "cold_load",
				receivedGeneration: 7,
				currentGeneration: 7,
				toolCatalogEligible: eligible,
			});
			capturedOptions?.recordMCPManifestUpdate?.({
				workspaceId: "default",
				sessionId: "sesn_manifest_log",
			...
			expect(logged).toEqual([
				expect.objectContaining({
					...
					"mcp.manifest.disposition": "applied",
					"mcp.manifest.source": "cold_load",
					"mcp.manifest.received_generation": 7,
					"mcp.manifest.current_generation": 7,
					"mcp.tool_catalog.eligible": true,
				}),
				expect.objectContaining({
					event: "runtime_mcp_manifest_update",
					"mcp.manifest.disposition": "stale",
					"mcp.manifest.source": "runtime_config_update",
					"mcp.manifest.received_generation": 6,
					"mcp.manifest.current_generation": 7,
					"mcp.tool_catalog.eligible": true,
				}),
			]);
	...
	test("command runner starts production dependency graph and closes resources on shutdown", async () => {
		...
		expect(capturedConfig?.mcpConnectorGrpcAddress).toBe(
			"gateway.engine.svc:9091",
		);
	...
	test("runtime config patch payload installs approval mode and provider-visible tool configs", () => {
		const policy = runtimeToolPolicyFromPatchPayload(
			JSON.stringify({
				config_generation: 7,
				runtime_config: {
	...
	test("cold runtime policy rejects absent, invalid, and ambiguous builtin family declarations", () => {
		...
		expect(() => runtimeToolPolicyFromPatchPayload(undefined)).toThrow(
			"runtime installed builtin family is malformed",
		);
	});
	...
	test("runtime policy aggregates admission config and MCP manifest tool projection", () => {
		...
		expect(policy.approvalMode).toBe("full_access");
		expect(lookupToolEntry(policy.toolCatalog, "Write")).toBeUndefined();
		expect(githubSearch?.route).toEqual({
			kind: "gateway",
		...
		expect(
			githubSearch !== undefined
				? effectivePermissionPolicy(githubSearch, policy.toolCatalog.configs)
				: undefined,
		).toBe("always_allow");
		expect(
			lookupToolEntry(policy.toolCatalog, "github_disabled"),
		).toBeUndefined();
	...
	test("manifest intake ignores inline policy and cannot bypass the held MCP toolset carrier", () => {
		...
		expect(
			lookupToolEntry(policy.toolCatalog, "github_search"),
		).toBeUndefined();
	});
	...
	test("unready MCP manifest supersedes the catalog with zero tools and hides its diagnostic", () => {
		...
		expect(lookupToolEntry(policy.toolCatalog, "Read")?.route.kind).toBe(
			"sandbox",
		);
		expect(JSON.stringify(policy.toolCatalog)).not.toContain(
			"delivery_exhausted",
		);
	});
	...
	test("rejects an unready MCP manifest whose tools field is not an empty array", () => {
		...
		expect(() => runtimeToolPolicyFromPatchPayloads(payloads)).toThrow(
			"invalid MCP manifest runtime config payload",
		);
	});
	...
	test("runtime policy gives approval reviewer threads only read-only approval-free tools", () => {
		for (const family of ["claude", "gpt"] as const) {
							...
							memoryStores: [
								{
									...
									access: "read_only",
									instructions:
										"Never expose this memory guidance to the reviewer.",
								},
							],
						},
						approval_mode: "approve_for_me",
			...
			expect(policy.toolCatalog.entries.map((entry) => entry.name)).toEqual([
				"Read",
				"Grep",
				"Glob",
			]);
			...
			expect(
				read !== undefined
					? effectivePermissionPolicy(read, policy.toolCatalog.configs)
					: undefined,
			).toBe("always_allow");
		}
	});
	...
	test("runtime model resolution accepts both payload key spellings like its sibling parsers", () => {
		...
		expect(
			runtimeModelForThread(
				...
				[
					JSON.stringify({
						runtimeConfig: { agent: { config: { model: "openai/gpt-5.5" } } },
					}),
				],
				reviewerModel,
			),
		...
		expect(
			runtimeModelForThread(
				...
				[
					JSON.stringify({
						runtime_config: { agent: { config: { model: "openai/gpt-5.5" } } },
					}),
				],
				reviewerModel,
			),
	...
	test("production dependency builder wires core hosts, TokenReview auth, and app command path", async () => {
		...
		const outboundInternalGrpcTokenPath = join(
			dir,
			"outbound-internal-grpc-token",
		);
		const tokenReviewReviewerTokenPath = join(
			dir,
			"token-review-reviewer-token",
		);
		...
		try {
			...
			const tokenReviewRequest = requireTokenReviewRequest(
			...
			expect(
				(tokenReviewRequest.headers as Record<string, string>).authorization,
			).toBe("bearer reviewer-token");
			...
			expect(
				ca !== undefined &&
					...
					? ca.name
					: undefined,
			).toBe(kubernetesApiCaCertPath);
			expect(tokenReviewReviewerTokenPath).not.toBe(
				outboundInternalGrpcTokenPath,
			);
		} finally {
	...
	test("production startup validates inbound TokenReview reviewer token and CA material before readiness", async () => {
		for (const scenario of [
			...
			try {
				await expect(dependencies.app.start(), scenario.name).rejects.toThrow(
					"runtime pod startup failed",
				);

				const serialized = JSON.stringify(records);
				expect(dependencies.app.lifecycle.ready(), scenario.name).toEqual({
					ready: false,
				});
	...
	test("command runner classifies dependency construction failures as startup_error", async () => {
				...
				runRuntimePodCommand({
					logger: {
						...
						error: (record) => {
							...
							records.push(
								`${record.kind}:${record.message}:${record["error.class"]}:${record["error.code"]}:${record["error.message_safe"]}`,
							);
						},
					},
					dependencyBuilder: async () => {
						throw new Error(
							`bearer secret-token https://kubernetes.default.svc {"kind":"TokenReview"} raw request body sk-provider-key kube object dump`,
						);
					},
		...
		expect(records[1]).toBe(
			"startup_error:runtime pod startup failed:startup_error:startup_error:runtime pod startup failed",
		);
		for (const forbidden of [
			"secret-token",
			"kubernetes.default.svc",
...
async function tokenReviewMaterialFixture(scenario: {
	...
	const tokenPath = await materialPath(
		dir,
		"reviewer-token",
		scenario.token,
		`${tokenCanary}\n`,
	);
	const caPath = await materialPath(
		dir,
		"ca.crt",
		scenario.ca,
		`-----BEGIN CERTIFICATE-----\n${caCanary}\n-----END CERTIFICATE-----\n`,
	...
	return {
		...
		coreHosts: {
			...
			subAgentRunHost: {
				...
				evictReviewerExecution: async (command) => ({
					ok: true,
					sessionId: command.sessionId,
				...
				interruptReviewerExecution: async (command) => ({
					ok: true,
					sessionId: command.sessionId,
					sessionThreadId: command.sessionThreadId,
					applied: true,
				...
				releaseReviewerExecution: async (command) => ({
					ok: true,
					sessionId: command.sessionId,
					sessionThreadId: command.sessionThreadId,
					applied: true,
				...
				waitReviewerExecution: async (command) => ({
					ok: true,
					sessionId: command.sessionId,
					sessionThreadId: command.sessionThreadId,
					status: "idle",
				...
				inspectReviewerExecution: async (command) => ({
					ok: false,
					sessionId: command.sessionId,
					sessionThreadId: command.sessionThreadId,
				...
				commitApprovalReviewFailure: async () => ({
					ok: true,
					type: "committed",
					eventId: "evt_failure",
					eventSequence: 1,
