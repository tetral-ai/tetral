...
import { ProviderGatewayServiceService } from "@tetral/gateway-protocol/src/gen/tetral/provider_gateway/v1/provider_gateway.js";
...
describe("Gateway completion and durable interrupt linearization", () => {
	test("durable interrupt wins before the completion deadline", async () => {
		...
		try {
			...
			beginTestUserInterrupt(
				fixture.session,
				"rin_interrupt_before_gateway_deadline",
			);
			const interrupted = fixture.interruptRun();
			await waitForCondition(
				() => fixture.requestEnds.length === 1,
				"interrupted Request End",
			);
			fixture.deadline();
			...
			await waitForCondition(
				() => fixture.gateway.cancelledCalls() === 1,
				"single cancelled gRPC call",
			);
			expect(closeOutcomes(fixture.loggerRecords)).toEqual(["cancelled"]);
			expect(closeRecords(fixture.loggerRecords)[0]).toMatchObject({
				"cancel.kind": "caller",
			});
			...
			expect(JSON.stringify(fixture.loggerRecords)).not.toContain(
	...
	test("completion deadline wins before durable interrupt admission", async () => {
		...
		try {
			...
			beginTestUserInterrupt(
				fixture.session,
				"rin_interrupt_after_gateway_deadline",
			);
			const interrupted = fixture.interruptRun();
			fixture.releaseReschedule();
			...
			expect(closeOutcomes(fixture.loggerRecords)).toEqual([
				"completion_deadline",
			]);
			expect(streamRecordKinds(fixture.loggerRecords)).toEqual([
				"runtime_provider_stream_opened",
...
async function startRaceFixture(name: string): Promise<RaceFixture> {
		...
		entries: [
			{
				messageSequence: 1,
				contextKind: "user",
				parts: [{ type: "text", text: "raw provider payload marker" }],
			},
		],
	});
	...
	const llmService = createLLMService(
		client,
		runtimeProviderStreamObserver(logger, () => callClock),
	);
	...
	await waitForCondition(
		() => callbacks.length === 1,
		"Runtime completion timer",
	);
	expect(scheduledDurations).toEqual([
		25 + RuntimeGatewayTransportCompletionAllowanceMs,
	]);
	return {
...
function closeOutcomes(
	records: readonly RuntimePodLogRecord[],
): readonly unknown[] {
	return closeRecords(records).map((record) => record["transport.outcome"]);
}

function closeRecords(
	records: readonly RuntimePodLogRecord[],
): readonly RuntimePodLogRecord[] {
	return records.filter(
		(record) => record.event === "runtime_provider_stream_closed",
	);
}

function streamRecordKinds(
	records: readonly RuntimePodLogRecord[],
): readonly string[] {
	return records
