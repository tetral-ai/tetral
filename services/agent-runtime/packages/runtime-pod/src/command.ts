...
export interface RuntimePodDependencyBuilderOptions {
	...
	readonly outboundMetadataFactory?: (
...
function providerStreamTimeoutOptions(
	config: Pick<RuntimePodConfig, "providerStreamTimeoutMs">,
): {
...
export async function buildRuntimePodCommandDependencies(input: {
	...
	const outboundMetadataFactory =
		input.builderOptions?.outboundMetadataFactory ??
		buildOutboundBearerMetadata;
	...
	const coreHosts = await (
		...
		resolveMCPManifestEligibility: (effectivePatches, mcpServerName) =>
			runtimeMCPManifestEligibilityFromPatchPayloads(
				effectivePatches.map((patch) => patch.contentJson),
				mcpServerName,
			),
		threadLoop: {
			...
			providerCallRuntime: {
				...
				skillGuidanceDescriptionBudgetBytes:
					input.config.skillGuidance.descriptionBudgetBytes,
			},
			...
			acceptSandboxExecution: (request) =>
				toolRunner.acceptSandboxExecution(request),
			awaitSandboxExecution: (request) =>
				toolRunner.awaitSandboxExecution(request),
	...
	const controlInputCommitter =
		input.builderOptions?.controlInputCommitterFactory?.({
			config: input.config,
			metadataFactory: outboundMetadataFactory,
		}) ??
...
export function runtimeToolPolicyFromPatchPayload(
	...
	return runtimeToolPolicyFromPatchPayloads(
...
export function runtimeToolPolicyFromPatchPayloads(
	...
	return runtimeToolPolicyFromPatchPayloadsWithFamily(
		payloadJsons,
		undefined,
		true,
	);
...
export function runtimeMCPManifestEligibilityFromPatchPayloads(
	payloadJsons: readonly string[],
	...
	for (const payloadJson of payloadJsons) {
		...
		const manifest =
			recordField(parsed, "mcp_manifest") ?? recordField(parsed, "mcpManifest");
		if (!isRecord(manifest)) continue;
		const serverName =
			recordStringField(manifest, "mcp_server_name") ??
			recordStringField(manifest, "mcpServerName");
		if (serverName === mcpServerName)
			manifestReady =
				(recordStringField(manifest, "readiness") ?? "ready") === "ready";
	}
...
function runtimeToolPolicyFromPatchPayloadsWithFamily(
	...
	for (const [patchIndex, payloadJson] of payloadJsons.entries()) {
		...
		const runtimeConfig =
			recordField(parsed, "runtime_config") ??
			recordField(parsed, "runtimeConfig");
		...
		const nextSkillsIndexValue =
			recordField(runtimeConfig, "skillsIndex") ??
			recordField(runtimeConfig, "skills_index");
		if (nextSkillsIndexValue !== undefined) {
			skillsIndex = parseSkillGuidanceIndex(nextSkillsIndexValue);
		}
		const nextMemoryStoresValue =
			recordField(runtimeConfig, "memoryStores") ??
		...
		providerRescheduleBudget = parseRescheduleBudget(
			recordNumberField(runtimeConfig, "providerRescheduleBudget") ??
				recordNumberField(runtimeConfig, "provider_reschedule_budget"),
			providerRescheduleBudget,
		);
		compactionRescheduleBudget = parseRescheduleBudget(
			recordNumberField(runtimeConfig, "compactionRescheduleBudget") ??
				recordNumberField(runtimeConfig, "compaction_reschedule_budget"),
		...
		if (mcpManifest !== undefined) {
			const configuredManifest = applyMcpToolsetConfig(
				mcpManifest,
				mcpToolsets,
			);
...
function parseMemoryStores(value: unknown): readonly MemoryStorePromptEntry[] {
	...
	return value.map((item): MemoryStorePromptEntry => {
		...
		const memoryStoreId =
			recordStringField(item, "memoryStoreId") ??
			recordStringField(item, "memory_store_id");
...
function requiredColdInstalledBuiltinFamily(
	family: InstalledBuiltinFamily | undefined,
): InstalledBuiltinFamily {
	if (family === undefined) {
...
function unambiguousColdInstalledBuiltinFamily(
	...
	const installedTools =
		recordField(runtimeConfig, "installedTools") ??
...
export function runtimeModelForThread(
	...
	for (const payloadJson of payloadJsons) {
		...
		const runtimeConfig =
			recordField(payload, "runtime_config") ??
			recordField(payload, "runtimeConfig");
		if (!isRecord(runtimeConfig)) {
			continue;
		}
...
function runtimeAgentSystemPatch(
	...
	for (const source of [payload, runtimeConfig]) {
		if (!isRecord(source) || !Object.hasOwn(source, "system")) {
			continue;
		}
...
function parseRescheduleBudget(
...
function parseSkillGuidanceIndex(
	value: unknown,
): readonly SkillGuidanceIndexEntry[] {
	if (!Array.isArray(value)) {
...
class BridgeInternalToolRepairStore extends RuntimeInternalToolRepairStore {
	constructor(
		private readonly internalToolRepairCommitter: RuntimeInternalToolRepairCommitter,
	) {
		super();
	}
	...
	protected async commitInternalToolRepairRecord(
		...
		return await this.internalToolRepairCommitter.commitInternalToolRepair(
			repair,
		);
	}
}

function parseRuntimePolicyPayload(
	payloadJson: string | undefined,
...
function parseMcpManifest(
	payload: Record<string, unknown>,
): MCPManifest | undefined {
	const manifest =
		recordField(payload, "mcp_manifest") ?? recordField(payload, "mcpManifest");
	...
	const mcpServerName =
		...
		recordStringField(manifest, "mcpServerName");
	const manifestETag =
		recordStringField(manifest, "manifest_etag") ??
		recordStringField(manifest, "manifestETag");
	const manifestGeneration =
		recordNumberField(manifest, "manifest_generation") ??
		recordNumberField(manifest, "manifestGeneration");
	...
	const readinessShape =
		rawReadiness === undefined || typeof rawReadiness === "string";
	const diagnosticShape =
		rawDiagnostic === undefined ||
		rawDiagnostic === null ||
		typeof rawDiagnostic === "string";
	if (readiness === "unready") {
		if (
			mcpServerName === undefined ||
			etagPresent ||
			manifestGeneration === undefined ||
			!Number.isSafeInteger(manifestGeneration) ||
			manifestGeneration <= 0 ||
			diagnostic === undefined ||
			!readinessShape ||
			!diagnosticShape ||
			(rawToolsValue !== undefined &&
				(!Array.isArray(rawToolsValue) || rawToolsValue.length > 0))
		) {
			throw new Error("invalid MCP manifest runtime config payload");
		}
		return undefined;
...
function parseMcpToolsets(
	values: readonly unknown[] | undefined,
): ReadonlyMap<string, MCPManifestToolsetConfig> | undefined {
	if (values === undefined) {
		return undefined;
	}
	...
	for (const value of values) {
		...
		const mcpServerName =
			recordStringField(value, "mcp_server_name") ??
			recordStringField(value, "mcpServerName");
		if (mcpServerName === undefined) {
			throw new Error("invalid MCP toolset runtime config payload");
		}
		toolsets.set(mcpServerName, {
			mcpServerName,
			defaultConfig: parseMcpDefaultConfig(
				recordField(value, "default_config") ??
					recordField(value, "defaultConfig"),
			),
...
function parseMcpManifestTools(
	...
	for (const value of values) {
		...
		const extraFields = Object.keys(value).filter(
			(field) =>
				field !== "name" && field !== "description" && field !== "input_schema",
		);
...
function parseMcpDefaultConfig(value: unknown): MCPManifest["defaultConfig"] {
	...
	const permissionPolicy = parsePermissionPolicy(
		value.permission_policy ?? value.permissionPolicy,
	);
	return enabled === undefined && permissionPolicy === undefined
		? undefined
		: { enabled, permissionPolicy };
}

function parseMcpToolConfigs(
	values: readonly unknown[],
): readonly MCPManifestToolConfig[] {
