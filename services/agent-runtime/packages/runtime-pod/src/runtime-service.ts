 ...
 * universal response receipt crosses the boundary.
 */
...
export type RuntimeMessagesCommand = RuntimeInputScope & {
...
export interface RuntimeSessionRunHost {
  readonly handleAcceptInput: (command: RuntimeMessagesCommand) => Promise<
    | { readonly ok: true; readonly sessionId: string; readonly created: boolean; readonly started: boolean; readonly duplicate?: true | undefined }
    | { readonly ok: false; readonly sessionId: string; readonly reason: "local_session_capacity_exceeded" | "control_conflict" | "context_load_failed"; readonly retryable?: boolean | undefined }
  >;
  ...
  readonly handleInterruptControl: (
    ...
    | { readonly ok: false; readonly sessionId: string; readonly reason: "local_session_capacity_exceeded" | "control_busy" | "context_load_failed"; readonly retryable?: boolean | undefined; readonly errorCode?: string | number | undefined }
  >;
  ...
  readonly handleRuntimeConfigPatch: (
    ...
    | { readonly ok: true; readonly sessionId: string; readonly created: boolean; readonly applied: boolean; readonly noResidency?: true | undefined }
    | { readonly ok: false; readonly sessionId: string; readonly reason: "local_session_capacity_exceeded" | "control_busy" | "control_conflict" | "context_load_failed" }
  >;
}
...
export interface RuntimeCleanupController {
  readonly startCleanup: (scope: RuntimeCleanupCommand, reason: "expired" | "operator_requested") => Promise<
    | { readonly ok: true; readonly sessionId: string; readonly completion: Promise<{ readonly ok: true; readonly sessionId: string; readonly cleaned: boolean }> }
...
export interface RuntimeControlServiceOptions {
  readonly ownPod: { readonly namespace: string; readonly name: string; readonly uid: string; readonly ip: string };
...
const Methods = {
  ...
  acceptAgentMail: "/tetral.agent_runtime.v1.AgentRuntimePodService/AcceptAgentMail",
  acceptTaskNotification: "/tetral.agent_runtime.v1.AgentRuntimePodService/AcceptTaskNotification",
  interrupt: "/tetral.agent_runtime.v1.AgentRuntimePodService/Interrupt",
  resolveToolConfirmation: "/tetral.agent_runtime.v1.AgentRuntimePodService/ResolveToolConfirmation",
  applyRuntimeConfig: "/tetral.agent_runtime.v1.AgentRuntimePodService/ApplyRuntimeConfig",
  cleanupSession: "/tetral.agent_runtime.v1.AgentRuntimePodService/CleanupSession",
} as const;
...
interface MethodExecution<Request extends RuntimeSessionScope, Response> {
  ...
  readonly identity: () => string;
  readonly validate: (request: Request) => { readonly ok: boolean; readonly message?: string };
  ...
  readonly rejectionDiagnostic?: ((response: Response) => IngressRejectionDiagnostic) | undefined;
...
export class RuntimeControlService {
  ...
  private readonly configContentMemo = new Map<string, Map<string, RuntimeConfigMemoEntry>>();
  ...
  async acceptInput(request: AcceptInputRequest, metadata: Metadata): Promise<AcceptInputResponse> {
    ...
    return await this.executeMethod({
      ...
      selectedPodRejected: () => acceptInputRejected(AcceptInputFailure.ACCEPT_INPUT_FAILURE_SELECTED_POD_MISMATCH, true),
      bindingRejected: () => acceptInputRejected(AcceptInputFailure.ACCEPT_INPUT_FAILURE_BINDING_MISMATCH, true),
      identityRejected: () => acceptInputRejected(AcceptInputFailure.ACCEPT_INPUT_FAILURE_IDENTITY_CONFLICT, false),
      duplicate: (response) => response.rejected === undefined ? { duplicate: {} } : response,
      rejected: (response) => response.rejected !== undefined,
      apply: async () => {
        const result = await this.options.runHost.handleAcceptInput(messagesCommand(request, scope));
        if (!result.ok) {
          if (result.reason === "control_conflict") {
            return acceptInputRejected(AcceptInputFailure.ACCEPT_INPUT_FAILURE_IDENTITY_CONFLICT, false);
          }
          if (result.reason === "context_load_failed") {
            return acceptInputRejected(AcceptInputFailure.ACCEPT_INPUT_FAILURE_CONTEXT_LOAD_FAILED, result.retryable ?? false);
          }
  ...
  async acceptAgentMail(request: AcceptAgentMailRequest, metadata: Metadata): Promise<AcceptAgentMailResponse> {
    const scope = inputScope(request);
    return await this.executeMethod({
      request,
      metadata,
      ...
      selectedPodRejected: () => acceptAgentMailRejected(AcceptAgentMailFailure.ACCEPT_AGENT_MAIL_FAILURE_SELECTED_POD_MISMATCH, true),
      bindingRejected: () => acceptAgentMailRejected(AcceptAgentMailFailure.ACCEPT_AGENT_MAIL_FAILURE_BINDING_MISMATCH, true),
      identityRejected: () => acceptAgentMailRejected(AcceptAgentMailFailure.ACCEPT_AGENT_MAIL_FAILURE_IDENTITY_CONFLICT, false),
      duplicate: (response) => response.rejected === undefined ? { duplicate: {} } : response,
      rejected: (response) => response.rejected !== undefined,
      apply: async () => {
        const message = runtimeMessageFromAgentMailContent({
        ...
        if (!result.ok) {
          ...
          const reason = result.reason === "thread_not_receivable"
          ...
          return acceptAgentMailRejected(reason, result.reason === "context_load_failed" ? result.retryable ?? false : false);
        }
  ...
  async acceptTaskNotification(request: AcceptTaskNotificationRequest, metadata: Metadata): Promise<AcceptTaskNotificationResponse> {
    const scope = inputScope(request);
    return await this.executeMethod({
      request,
      metadata,
      ...
      selectedPodRejected: () => acceptTaskRejected(AcceptTaskNotificationFailure.ACCEPT_TASK_NOTIFICATION_FAILURE_SELECTED_POD_MISMATCH, true),
      bindingRejected: () => acceptTaskRejected(AcceptTaskNotificationFailure.ACCEPT_TASK_NOTIFICATION_FAILURE_BINDING_MISMATCH, true),
      identityRejected: () => acceptTaskRejected(AcceptTaskNotificationFailure.ACCEPT_TASK_NOTIFICATION_FAILURE_IDENTITY_CONFLICT, false),
      duplicate: (response) => response.rejected === undefined ? { duplicate: {} } : response,
      rejected: (response) => response.rejected !== undefined,
      apply: async () => {
        ...
        if (!result.ok) {
          return acceptTaskRejected(taskFailure(result.reason), result.reason === "control_busy" || result.reason === "context_load_failed");
        }
        return result.applied ? { accepted: {} } : { duplicate: {} };
      },
      acceptedBinding: true,
  ...
  async interrupt(request: InterruptRequest, metadata: Metadata): Promise<InterruptResponse> {
    const scope = inputScope(request);
    return await this.executeMethod({
      request,
      metadata,
      ...
      selectedPodRejected: () => interruptRejected(InterruptFailure.INTERRUPT_FAILURE_SELECTED_POD_MISMATCH, true),
      bindingRejected: () => interruptRejected(InterruptFailure.INTERRUPT_FAILURE_BINDING_MISMATCH, true),
      identityRejected: () => interruptRejected(InterruptFailure.INTERRUPT_FAILURE_IDENTITY_CONFLICT, false),
      duplicate: (response) => response.rejected === undefined ? { duplicate: {} } : response,
      rejected: (response) => response.rejected !== undefined,
      apply: async () => {
        ...
        const result = await this.options.runHost.handleInterruptControl(
          request.sessionId,
          {
            ...scope,
            inputOrder: request.inputOrder,
            origin: request.origin === InterruptOrigin.INTERRUPT_ORIGIN_AGENT ? "agent" : "user",
          },
          async (declaration) => {
            committed = await this.commitControlInput(scope, "interrupt_control", declaration);
            return committed;
          },
        );
        if (!result.ok) {
          if (committed?.ok === false) {
            return interruptRejected(commitInterruptFailure(committed.errorCode), committed.retryable);
          }
          if (result.reason === "local_session_capacity_exceeded") {
            throw new GrpcStatusError(status.RESOURCE_EXHAUSTED, "local runtime capacity exceeded");
          }
  ...
  async resolveToolConfirmation(request: ResolveToolConfirmationRequest, metadata: Metadata): Promise<ResolveToolConfirmationResponse> {
    const scope = inputScope(request);
    return await this.executeMethod({
      request,
      metadata,
      ...
      selectedPodRejected: () => resolveToolRejected(ResolveToolConfirmationFailure.RESOLVE_TOOL_CONFIRMATION_FAILURE_SELECTED_POD_MISMATCH, true),
      bindingRejected: () => resolveToolRejected(ResolveToolConfirmationFailure.RESOLVE_TOOL_CONFIRMATION_FAILURE_BINDING_MISMATCH, true),
      identityRejected: () => resolveToolRejected(ResolveToolConfirmationFailure.RESOLVE_TOOL_CONFIRMATION_FAILURE_IDENTITY_CONFLICT, false),
      duplicate: (response) => response.rejected === undefined ? { duplicate: {} } : response,
      rejected: (response) => response.rejected !== undefined,
      apply: async () => {
        let committed: RuntimeControlInputCommitResult | undefined;
        const result = await this.options.runHost.handleToolConfirmation(request.sessionId, {
          ...scope,
          toolUseEventId: request.toolUseEventId,
          decision: request.decision === ToolConfirmationDecision.TOOL_CONFIRMATION_DECISION_DENY ? "deny" : "allow",
          ...(request.denyMessage === undefined ? {} : { denyMessage: request.denyMessage }),
        }, async (declaration) => {
          committed = await this.commitControlInput(scope, "tool_confirmation", declaration);
          return committed;
        });
        if (!result.ok && committed?.ok === false) {
          return resolveToolRejected(commitToolFailure(committed.errorCode), committed.retryable);
        }
        if (!result.ok) {
          if (result.reason === "local_session_capacity_exceeded") {
            throw new GrpcStatusError(status.RESOURCE_EXHAUSTED, "local runtime capacity exceeded");
  ...
  async applyRuntimeConfig(request: ApplyRuntimeConfigRequest, metadata: Metadata): Promise<ApplyRuntimeConfigResponse> {
    ...
    return await this.executeMethod({
      ...
      selectedPodRejected: () => applyConfigRejected(ApplyRuntimeConfigFailure.APPLY_RUNTIME_CONFIG_FAILURE_SELECTED_POD_MISMATCH, true),
      bindingRejected: () => applyConfigRejected(ApplyRuntimeConfigFailure.APPLY_RUNTIME_CONFIG_FAILURE_BINDING_MISMATCH, true),
      identityRejected: () => applyConfigRejected(ApplyRuntimeConfigFailure.APPLY_RUNTIME_CONFIG_FAILURE_IDENTITY_CONFLICT, false),
      duplicate: (result) => result.rejected === undefined ? { duplicate: {} } : result,
      rejected: (result) => result.rejected !== undefined,
      rejectionDiagnostic: (result) => result.rejected?.reason === ApplyRuntimeConfigFailure.APPLY_RUNTIME_CONFIG_FAILURE_IDENTITY_CONFLICT
      ...
      apply: async () => {
        ...
        if (
          ...
          return applyConfigRejected(ApplyRuntimeConfigFailure.APPLY_RUNTIME_CONFIG_FAILURE_IDENTITY_CONFLICT, false);
        }
        const result = await this.options.runHost.handleRuntimeConfigPatch(request.sessionId, command);
        if (!result.ok) {
          return applyConfigRejected(configFailure(result.reason), result.reason === "control_busy" || result.reason === "context_load_failed");
        }
        if (result.noResidency === true) {
          this.deleteRuntimeConfigMemo(memoSessionKey, memoFamily);
          return { noResidency: {} };
        }
        const nextSessionMemo = sessionMemo ?? new Map<string, RuntimeConfigMemoEntry>();
  ...
  async cleanupSession(request: CleanupSessionRequest, metadata: Metadata): Promise<CleanupSessionResponse> {
    ...
    return await this.executeMethod({
      ...
      selectedPodRejected: () => cleanupRejected(CleanupSessionFailure.CLEANUP_SESSION_FAILURE_SELECTED_POD_MISMATCH, true),
      bindingRejected: () => cleanupRejected(CleanupSessionFailure.CLEANUP_SESSION_FAILURE_BINDING_MISMATCH, true),
      identityRejected: () => cleanupRejected(CleanupSessionFailure.CLEANUP_SESSION_FAILURE_CLEANUP_FAILED, false),
      duplicate: (response) => response.rejected === undefined ? { duplicate: {} } : response,
      rejected: (response) => response.rejected !== undefined,
      apply: async () => {
        const cleanup = await this.options.cleanupController.startCleanup(
          scope,
          request.reason === CleanupSessionReason.CLEANUP_SESSION_REASON_EXPIRED ? "expired" : "operator_requested",
        );
        if (!cleanup.ok) {
          runtimeMetrics(this.options).recordCleanupCommandOutcome("rejected");
          return cleanupRejected(CleanupSessionFailure.CLEANUP_SESSION_FAILURE_SESSION_BUSY, true);
        }
        ...
        try {
          ...
          return cleanupRejected(CleanupSessionFailure.CLEANUP_SESSION_FAILURE_CLEANUP_FAILED, true);
        }
      },
  ...
  private async executeMethod<Request extends RuntimeSessionScope, Response>(execution: MethodExecution<Request, Response>): Promise<Response> {
    ...
    try {
      ...
      this.logOutcome(execution, undefined, startedAt, {
    ...
    const recordRejection = (diagnostic: IngressRejectionDiagnostic, identityTrusted: boolean): void => {
      rejectionRecorded = true;
      this.logOutcome(execution, caller, startedAt, diagnostic, identityTrusted);
    };
    const run = async (lease?: RuntimeCommandLease): Promise<Response> => {
      try {
        ...
        recordRejection({ phase: "lifecycle", reason: "runtime_not_accepting", grpcCode: grpcCodeText(error) }, false);
        throw error;
      }
      const validation = execution.validate(execution.request);
      if (!validation.ok) {
        recordRejection({ phase: "request_validation", reason: "invalid_request", grpcCode: "InvalidArgument" }, false);
        throw new GrpcStatusError(status.INVALID_ARGUMENT, validation.message ?? "invalid internal request");
      }
      if (execution.request.targetPodUid !== this.options.ownPod.uid) {
        const response = execution.selectedPodRejected();
        recordRejection({ phase: "selected_pod", reason: "selected_pod_mismatch" }, true);
        return response;
      }
      ...
      const existing = this.inFlight.get(execution.dedupeKey) as InFlight<Response> | undefined;
      if (existing !== undefined) {
        if (existing.identity !== identity) {
          const response = execution.identityRejected();
          recordRejection({ phase: "identity", reason: "identity_conflict" }, true);
          return response;
        }
        ...
        if (execution.rejected(response)) {
          recordRejection(execution.rejectionDiagnostic?.(response) ?? {
            phase: "application",
      ...
      try {
        ...
        lease?.throwIfAborted();
        if (!execution.rejected(response)) {
          this.recordAcceptedBinding(execution.request, execution.acceptedBinding);
        }
        this.inFlight.delete(execution.dedupeKey);
    ...
    try {
      return this.options.commandRunner === undefined
        ? await run()
        : await this.options.commandRunner.runCommand(async (lease) => await run(lease));
    } catch (error) {
      if (!rejectionRecorded) {
        recordRejection({ phase: "lifecycle", reason: "runtime_not_accepting", grpcCode: grpcCodeText(error) }, false);
      }
  ...
  private async commitControlInput(
    ...
    if (!result.ok) {
      return { ok: false, retryable: result.retryable, errorCode: result.errorCode };
    }
    return "stale" in result ? { ok: true, stale: true } : { ok: true, receipt: result.receipt };
  }

  private deleteRuntimeConfigMemo(memoSessionKey: string, memoFamily: string): void {
    const sessionMemo = this.configContentMemo.get(memoSessionKey);
  ...
  private async authorize(metadata: Metadata, method: string): Promise<{ readonly serviceAccount: ServiceAccountIdentity }> {
    ...
    if (result.serviceAccount.namespace !== this.options.allowedBridge.namespace || result.serviceAccount.name !== this.options.allowedBridge.name) {
  ...
  private activeBindingMatches(scope: RuntimeSessionScope): boolean {
    ...
    return active === undefined || (active.bindingId === scope.bindingId && active.bindingGeneration === scope.bindingGeneration);
  }

  private recordAcceptedBinding(scope: RuntimeSessionScope, retain: boolean): void {
    if (!retain) {
  ...
  private logOutcome<Request extends RuntimeSessionScope, Response>(
    ...
    const errorFields = rejection !== undefined
      ? semanticErrorFields({ errorClass: "runtime_command_rejected", errorCode: rejection.reason, messageSafe: "runtime command rejected" })
      : {};
    try {
      this.options.logger.info({
        event: rejected ? "runtime_command_rejected" : "runtime_command_accepted",
        "event.kind": rejected ? "runtime_command_rejected" : "runtime_command_accepted",
        operation: execution.operation,
        component: "runtime-control-service",
        "grpc.method": execution.method,
        "grpc.code": rejection?.grpcCode ?? "OK",
        ...(caller === undefined ? {} : { "caller.service_account": serviceAccountText(caller.serviceAccount) }),
        "duration.ms": Date.now() - startedAt,
        ...(identityTrusted
          ? {
              "workspace.id": execution.request.workspaceId,
              "session.id": execution.request.sessionId,
              ...(hasThread(execution.request) ? { "thread.id": execution.request.sessionThreadId } : {}),
              "operation.id": execution.operationId,
              "binding.id": execution.request.bindingId,
            }
          : {}),
        ...(rejection === undefined ? {} : { phase: rejection.phase, reason: rejection.reason }),
        ...errorFields,
      });
    } catch {
...
function messagesCommand(request: AcceptInputRequest, scope: RuntimeInputScope): RuntimeMessagesCommand {
  if (request.rejection !== undefined) {
    return {
      ...scope,
      inputOrder: request.inputOrder,
      kind: "rejection",
      reasonCode: request.rejection.reason === AcceptInputRejectionReason.ACCEPT_INPUT_REJECTION_REASON_PAYLOAD_TOO_LARGE
...
function taskNotificationContent(notificationJson: string): {
  ...
  if (
    ...
    sourceToolUseEventId === undefined || sourceToolUseEventId.length === 0 ||
    statusValue !== "completed" &&
    statusValue !== "failed" &&
    statusValue !== "cancelled" &&
    statusValue !== "expired"
  ) {
    throw new GrpcStatusError(status.INVALID_ARGUMENT, "invalid task notification content");
  }
  for (const field of ["stdout", "stderr"] as const) {
    const value = payload[field];
    if (!isObjectRecord(value) || typeof value.text !== "string" || typeof value.truncated !== "boolean") {
      throw new GrpcStatusError(status.INVALID_ARGUMENT, "invalid task notification content");
    }
  }
...
function runtimeConfigCommand(request: ApplyRuntimeConfigRequest, scope: RuntimeSessionScope): RuntimeConfigCommand {
  ...
  const manifestPayload = isObjectRecord(payload.mcp_manifest) ? payload.mcp_manifest : payload;
  ...
  if (readinessValue !== "ready" && readinessValue !== "unready") {
    throw new GrpcStatusError(status.INVALID_ARGUMENT, "invalid runtime config content");
  }
...
function runtimeConfigMemoFamily(command: RuntimeConfigCommand): string {
  return command.mcpServerName === undefined ? "session" : `mcp:${command.mcpServerName}`;
}
...
function sessionKey(input: { readonly workspaceId: string; readonly sessionId: string }): string {
...
function acceptInputRejected(reason: AcceptInputFailure, retryable: boolean): AcceptInputResponse {
  return { rejected: { reason, retryable } };
}

function acceptAgentMailRejected(reason: AcceptAgentMailFailure, retryable: boolean): AcceptAgentMailResponse {
  return { rejected: { reason, retryable } };
}

function acceptTaskRejected(reason: AcceptTaskNotificationFailure, retryable: boolean): AcceptTaskNotificationResponse {
  return { rejected: { reason, retryable } };
}

function interruptRejected(reason: InterruptFailure, retryable: boolean): InterruptResponse {
  return { rejected: { reason, retryable } };
}

function resolveToolRejected(reason: ResolveToolConfirmationFailure, retryable: boolean): ResolveToolConfirmationResponse {
  return { rejected: { reason, retryable } };
}

function applyConfigRejected(reason: ApplyRuntimeConfigFailure, retryable: boolean): ApplyRuntimeConfigResponse {
  return { rejected: { reason, retryable } };
}

function cleanupRejected(reason: CleanupSessionFailure, retryable: boolean): CleanupSessionResponse {
  return { rejected: { reason, retryable } };
}

function taskFailure(reason: string): AcceptTaskNotificationFailure {
  switch (reason) {
    case "control_busy": return AcceptTaskNotificationFailure.ACCEPT_TASK_NOTIFICATION_FAILURE_CONTROL_BUSY;
    case "control_conflict": return AcceptTaskNotificationFailure.ACCEPT_TASK_NOTIFICATION_FAILURE_CONTROL_CONFLICT;
    case "context_load_failed": return AcceptTaskNotificationFailure.ACCEPT_TASK_NOTIFICATION_FAILURE_CONTEXT_LOAD_FAILED;
    default: throw new GrpcStatusError(status.RESOURCE_EXHAUSTED, "local runtime capacity exceeded");
  }
}

function configFailure(reason: string): ApplyRuntimeConfigFailure {
  switch (reason) {
    case "control_busy": return ApplyRuntimeConfigFailure.APPLY_RUNTIME_CONFIG_FAILURE_CONTROL_BUSY;
    case "control_conflict": return ApplyRuntimeConfigFailure.APPLY_RUNTIME_CONFIG_FAILURE_CONTROL_CONFLICT;
    case "context_load_failed": return ApplyRuntimeConfigFailure.APPLY_RUNTIME_CONFIG_FAILURE_CONTEXT_LOAD_FAILED;
    default: throw new GrpcStatusError(status.RESOURCE_EXHAUSTED, "local runtime capacity exceeded");
  }
}

function commitInterruptFailure(errorCode: string | number): InterruptFailure {
  return typeof errorCode === "string" && (errorCode.includes("unavailable") || errorCode.includes("token"))
    ? InterruptFailure.INTERRUPT_FAILURE_DURABLE_COMMIT_UNAVAILABLE
    : InterruptFailure.INTERRUPT_FAILURE_DURABLE_COMMIT_REJECTED;
}

function commitToolFailure(errorCode: string | number): ResolveToolConfirmationFailure {
  return typeof errorCode === "string" && (errorCode.includes("unavailable") || errorCode.includes("token"))
...
function parseObjectContent(contentJson: string, kind: string): Record<string, unknown> {
  try {
    ...
    throw new GrpcStatusError(status.INVALID_ARGUMENT, `invalid ${kind} content`);
  }
}
...
function hasThread(value: RuntimeSessionScope): value is RuntimeThreadScope {
  return "sessionThreadId" in value && typeof value.sessionThreadId === "string";
}
...
function grpcRejectionDiagnostic(error: unknown): IngressRejectionDiagnostic {
  if (!(error instanceof GrpcStatusError)) {
    return { phase: "application", reason: "internal_failure", grpcCode: "Internal" };
  }
  switch (error.code) {
    case status.INVALID_ARGUMENT:
      return { phase: "content_parse", reason: "invalid_content", grpcCode: "InvalidArgument" };
    case status.RESOURCE_EXHAUSTED:
      return { phase: "application", reason: "resource_exhausted", grpcCode: "ResourceExhausted" };
    case status.UNAVAILABLE:
      return { phase: "application", reason: "operation_unavailable", grpcCode: "Unavailable" };
    case status.FAILED_PRECONDITION:
      return { phase: "lifecycle", reason: "runtime_not_accepting", grpcCode: "FailedPrecondition" };
    default:
      return { phase: "application", reason: "internal_failure", grpcCode: grpcCodeText(error) };
  }
}
...
function grpcCodeText(error: unknown): string {
  ...
  switch (error.code) {
    ...
    case status.INVALID_ARGUMENT: return "InvalidArgument";
    case status.FAILED_PRECONDITION: return "FailedPrecondition";
    case status.PERMISSION_DENIED: return "PermissionDenied";
    case status.RESOURCE_EXHAUSTED: return "ResourceExhausted";
