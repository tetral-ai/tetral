...
branches="$(gh api "repos/$GITHUB_REPOSITORY/environments/release/deployment-branch-policies?per_page=100")"
if [[ "$(jq -r '[.branch_policies[].name] | sort | join(",")' <<<"$branches")" != main ]]; then
  echo "release environment must permit exactly main" >&2
  exit 1
fi

secret_names="$(gh api "repos/$GITHUB_REPOSITORY/environments/release/secrets?per_page=100" --jq '[.secrets[].name] | sort')"
variable_names="$(gh api "repos/$GITHUB_REPOSITORY/environments/release/variables?per_page=100" --jq '[.variables[].name] | sort')"
...
packages="$(gh api 'orgs/tetral-ai/packages?package_type=container&per_page=100')"
...
for name in "${package_names[@]}"; do
  ...
  if [[ -z "$package" ]]; then
    missing+=("$name")
    jq -cn --arg name "$name" '{found:false,name:$name,organization:"tetral-ai",visibility:"",linked_repository_id:0,actions_repository_ids:[],existing_references:[],repository_token_can_read:true,repository_token_can_write:true,creation_authorized:true}' >> "$identities"
    continue
  fi
  visibility="$(jq -r .visibility <<<"$package")"
  linked="$(jq -r '.repository.id // 0' <<<"$package")"
  encoded="$(jq -rn --arg value "$name" '$value|@uri')"
  access="$(gh api "orgs/tetral-ai/packages/container/$encoded/repositories?per_page=100")"
  versions="$(gh api "orgs/tetral-ai/packages/container/$encoded/versions?per_page=100")"
  actions_ids="$(jq -c '[.[].id] | sort' <<<"$access")"
  references="$(jq -c '[.[].metadata.container.tags[]] | unique | sort' <<<"$versions")"
  jq -cn --arg name "$name" --arg visibility "$visibility" --argjson linked "$linked" --argjson actions "$actions_ids" --argjson references "$references" '{found:true,name:$name,organization:"tetral-ai",visibility:$visibility,linked_repository_id:$linked,actions_repository_ids:$actions,existing_references:$references,repository_token_can_read:true,repository_token_can_write:true,creation_authorized:false}' >> "$identities"
done
...
jq -s --argjson repository_id "$repository_id" '{schema:"tetral.release-package-preflight/v1",organization:"tetral-ai",repository_id:$repository_id,require_write:true,packages:.}' "$identities" > .release-preflight/packages.json
