...
acceptance. The final check must pass after all attempts. Recording and
promotion require a report finished within the previous seven days.
...
Legacy `tetral.rehearsal-evidence/v1` records without a report remain readable
for historical state reconstruction. They cannot authorize a new promotion.
An unpublished Candidate that already owns immutable legacy evidence must be
retired and reissued under a new version, then rehearsed with this protocol.
Do not overwrite its old evidence. Already published releases retain their
historical records.
...
promotion validation. It also rejects a failed sibling step and legacy-only
acceptance. These tests verify the protocol handoff; they are not a live
